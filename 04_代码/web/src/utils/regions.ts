/** 省级区划 → 地级市（含州/盟/地区）。「全选全国」取 Object.keys。 */
...
export const PROVINCES = Object.keys(CITY_MAP)

export const ALL_CITIES = Object.values(CITY_MAP).flat()

export function citiesOf(provinces: string[]): string[] {
  const out: string[] = []
  for (const p of provinces || []) {
    for (const c of CITY_MAP[p] || []) {
...
export const BAND_SIZES = ['SS', 'S', 'M', 'L', 'LL']
export const BELTS = ['腰带SS', '腰带S', '腰带M', '腰带L', '腰带LL']
...
export function kitCombos(p: any) {
  const extra = p?.extra || p || {}
  if (Array.isArray(extra.stdCombos) && extra.stdCombos.length) return extra.stdCombos
