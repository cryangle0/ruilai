  ...
  <el-dialog v-model="visible" :title="title" width="860px" destroy-on-close @closed="mode='view'">
    <template v-if="form && form.id && mode==='view'">
      ...
      <el-table :data="form.parts||[]" size="small" border>
        <el-table-column label="配件"><template #default="{row}">{{ prodName(row.partId) }}</template></el-table-column>
        <el-table-column prop="spec" label="规格" /><el-table-column prop="qty" label="数量" width="80" />
      </el-table>
    </template>
    <template v-else-if="form && form.id && mode==='audit'">
      <div class="alert alert-info">段号数量须等于标准+非标总数；配件不计 SN，可单独成单。两位管理员会签通过后立即完成。号段若已发给其他代理或已售出则判异常；已退回原厂的 SN 可再次出给代理（含原代理）。</div>
      <h4>标准品</h4>
      ...
      <h4>非标品（个性化） <el-button size="small" @click="form.customLines.push({ productId: form.lines?.[0]?.productId || 'P1', size: 'SS', belt: '腰带SS', qty: 1 })">+ 加行</el-button></h4>
      <div v-for="(line, i) in form.customLines" :key="'c'+i" class="audit-line">
        <div class="audit-prod">{{ prodName(line.productId) }} · {{ line.size }}+{{ line.belt }} ×
      ...
      <div v-for="(pt, i) in form.parts" :key="'p'+i" class="segment-row">
        <el-select v-model="pt.partId" filterable placeholder="配件" style="width:160px">
...
function segCount(s: string) {
  ...
  const from = bits[0] || ''
  const to = bits.length > 1 ? bits.slice(1).join('-') : from
  if (!from) return 0
  const a = Number(from.replace(/\D/g, '').slice(-8))
  const b = Number((to || from).replace(/\D/g, '').slice(-8))
  if (!a || !b || b < a) return from ? 1 : 0
  return b - a + 1
}
...
watch(() => [props.modelValue, props.row], async () => {
  ...
  if (props.row?.id) {
    form.value = { ...props.row, lines: [...(props.row.lines||[])], customLines: [...(props.row.customLines||[])], parts: [...(props.row.parts||[])] }
  } else {
...
async function cosign() {
  ...
  await api.cosign(form.value.id, segmentsPayload())
