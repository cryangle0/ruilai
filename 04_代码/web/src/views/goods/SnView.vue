  ...
  <div class="page">
    <div class="page-header is-compact">
      ...
      <div class="page-actions" v-if="canManageSn">
        <el-button type="primary" @click="genOpen=true">系统生成SN</el-button>
    ...
    <SearchPanel :model="query" @search="load" @reset="reset">
      ...
      <el-form-item>
        <el-select v-model="query.productId" placeholder="商品编码" clearable filterable style="width:160px">
      ...
      <el-form-item>
        <el-select v-model="query.tag" placeholder="标签筛选" clearable style="width:120px">
          <el-option label="已冻结" value="已冻结" /><el-option label="已退货" value="已退货" /><el-option label="个性化" value="个性化" />
        </el-select>
      </el-form-item>
      <el-form-item><span class="muted-label">出厂</span></el-form-item>
      <el-form-item>
      ...
      <el-form-item><span class="muted-label">销售</span></el-form-item>
      <el-form-item>
      ...
      <el-form-item><span class="muted-label">退货</span></el-form-item>
      <el-form-item>
    ...
    <DataTableShell :data="list" :loading="loading" :total="total" v-model:page="page" v-model:pageSize="pageSize" @row-click="open">
      ...
      <el-table-column prop="sizeCode" label="尺寸" width="70" />
      ...
      <el-table-column label="标签" min-width="120">
        <template #default="{row}">
          <span v-for="t in (row.tags||[])" :key="t" class="tag tag-orange" style="margin-right:4px">{{ t }}</span>
        ...
        <div>
          <div class="detail-grid" style="grid-template-columns:1fr 1fr">
            ...
            <div class="span-2"><span>标签</span>
              <span v-for="t in (cur.tags||[])" :key="t" class="tag tag-orange" style="margin-right:4px">{{ t }}</span>
              <span v-if="!(cur.tags||[]).length">—</span>
            </div>
          </div>
          <template v-if="cust">
          ...
          <div v-if="editing" class="note-list">
            <el-input v-for="(n,i) in sitNotes" :key="'s'+i" v-model="sitNotes[i]" />
            <el-button size="small" @click="sitNotes.push('')">+ 添加</el-button>
          </div>
          <p v-else class="muted">{{ sitNotes.filter(Boolean).join('；') || '—' }}</p>
          <h4 style="margin-top:12px">处理说明</h4>
          <div v-if="editing" class="note-list">
            <el-input v-for="(n,i) in procNotes" :key="'p'+i" v-model="procNotes[i]" />
            <el-button size="small" @click="procNotes.push('')">+ 添加</el-button>
          </div>
          <p v-else class="muted">{{ procNotes.filter(Boolean).join('；') || '—' }}</p>
    ...
    <el-dialog v-model="genOpen" title="系统内部生成 SN（12.3）" width="480px">
      <div class="alert alert-info">不再依赖外部系统：按日期流水在本系统生成号段，状态为「原厂在库」。</div>
      <el-form label-width="90px" style="margin-top:12px">
        <el-form-item label="所属一级">
          <el-select v-model="gen.l1Id" filterable style="width:100%">
            <el-option v-for="a in l1s" :key="a.id" :label="a.name" :value="a.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="商品">
          <el-select v-model="gen.productId" style="width:100%">
            <el-option v-for="p in kits" :key="p.id" :label="p.name" :value="p.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="尺寸">
          <el-select v-model="gen.size" style="width:100%">
            <el-option v-for="s in BAND_SIZES" :key="s" :label="s" :value="s" />
          </el-select>
        </el-form-item>
        <el-form-item label="腰带">
          <el-select v-model="gen.belt" style="width:100%">
            <el-option v-for="s in BELTS" :key="s" :label="s" :value="s" />
          </el-select>
        </el-form-item>
        <el-form-item label="生成数量"><el-input-number v-model="gen.qty" :min="1" :max="200" /></el-form-item>
      </el-form>
      <template #footer>
...
const sitNotes = ref<string[]>([])
const procNotes = ref<string[]>([])
...
const reL1 = ref('')
const gen = reactive({ l1Id: '', productId: '', size: 'M', belt: '腰带M', qty: 10 })
...
async function saveEdit() {
  cur.value = await api.updateSn(cur.value.sn, {
    ...
    situationNotes: sitNotes.value.filter(Boolean),
    processNotes: procNotes.value.filter(Boolean),
  })
...
async function doGen() {
  try {
    await ElMessageBox.confirm(`确认系统生成 ${gen.qty} 条 SN？`, '生成 SN 确认', { confirmButtonText: '确认生成' })
  } catch { return }
  const rows = await api.generateSn(gen)
  ElMessage.success(`已生成 ${rows.length} 条`)
  genOpen.value = false
  load()
}
...
onMounted(async () => {
  ...
  gen.l1Id = l1s.value[0]?.id
  gen.productId = kits.value[0]?.id
  imp.l1Id = gen.l1Id
  imp.productId = gen.productId
  reL1.value = gen.l1Id
