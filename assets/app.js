(() => {
  ...
  function productComboNote(p) {
    ...
    if (p.type === 'part') {
      return `配件可单独下单（无 SN，不占号段）。当前规格：${(p.sizes || []).join('/') || '—'}`;
    }
    if (p.type === 'single') {
  ...
  const RETURN_TYPES = [
    ...
    { id: 'user', label: '终端用户退货' },
    { id: 'self', label: '自行售后处理' },
  ];
  const SN_STATUS_LABEL = {
    warehouse: '原厂在库', l1: '一级在库', l2: '二级在库', bound: '已销售',
  };
  const SN_BIZ_TAGS = ['已退货', '再入库', '再销售', '已冻结'];
  const PO_STATUS = {
    pending: '待处理', cosigning: '会签中', approved: '已完成', rejected: '已删除',
  };
  function poStatusTag(status) {
    const tone = status === 'pending' ? 'orange'
      : status === 'cosigning' ? 'blue'
      : status === 'approved' ? 'green'
      : 'gray';
    return tag(PO_STATUS[status] || status || '—', tone);
  }
  const SO_STATUS = {
  ...
  function createdStamp(row) {
    return parseTime(row?.createdAt || row?.time || row?.factoryAt || row?.soldAt || row?.code || row?.id || row?.sn || '');
  }
  function sortByCreated(list) {
    return list.slice().sort((a, b) => {
      const d = createdStamp(a) - createdStamp(b);
      if (d) return d;
      return String(a.id || a.sn || a.code || '').localeCompare(String(b.id || b.sn || b.code || ''));
    });
  }
  function sortAgentsExFirst(rows, exFn) {
    return rows.slice().sort((a, b) => {
      const ea = exFn(a) > 0 ? 0 : 1;
      const eb = exFn(b) > 0 ? 0 : 1;
      if (ea !== eb) return ea - eb;
      const d = String(a.code || '').localeCompare(String(b.code || ''), 'zh');
      if (d) return d;
  ...
  function seed() {
    ...
    const agentsL1 = [
      ...
      },
      {
        ...
        disableCosign: { admin1: true, admin2: false, admin1At: '2026-08-07 09:40', admin1By: 'admin' },
    ...
    return {
      ...
      returns: [
        { id: 'RT1', no: `RTU${todayCompact()}01`, type: 'user', typeLabel: '终端用户退货', fromId: 'L2A', fromName: '杭州城西专营',
          sns: ['RL202607200007'], status: 'done', createdAt: '2026-08-01 11:20', reason: '尺码不合适', reasonType: '尺码' },
        { id: 'RT2', no: `RT${todayCompact()}02`, type: 'l2_to_l1', typeLabel: '二级退一级', fromId: 'L2A', fromName: '杭州城西专营', approverId: 'L1A',
          sns: ['RL202607200011'], status: 'done', createdAt: '2026-08-04 09:00', reason: '质量问题：面料起球', reasonType: '质量' },
        ...
        { id: 'RT5', no: `RT${todayCompact()}05`, type: 'l2_to_l1', typeLabel: '二级退一级', fromId: 'L2B', fromName: '宁波海曙店', approverId: 'L1A',
          sns: ['RL202607210006'], status: 'done', createdAt: '2026-07-30 16:40', reason: '客诉退货，转退一级', reasonType: '投诉' },
        ...
        { id: 'RT11', no: `RTU${todayCompact()}11`, type: 'user', typeLabel: '直售客户退货', fromId: 'L1A', fromName: '华东锐涞总代',
          sns: ['RL202608030001'], status: 'done', createdAt: '2026-08-05 16:40', reason: '尺码偏大，直售客户申请退货', reasonType: '尺码',
          customer: { name: '王芳', gender: '女', age: '31', phone: '13700001108', addr: '杭州市西湖区文三路100号', phoneLoc: '浙江' } },
        { id: 'RT12', no: `RTU${todayCompact()}12`, type: 'user', typeLabel: '直售客户退货', fromId: 'L1B', fromName: '华南渠道中心',
          sns: ['RL202608050001'], status: 'done', createdAt: '2026-08-06 09:30', reason: '面料起球，直售客户退货', reasonType: '质量',
          customer: { name: '刘洋', gender: '男', age: '34', phone: '13900002201', addr: '广州市天河区体育西路8号', phoneLoc: '广东' } },
        { id: 'RT13', no: `RTU${todayCompact()}13`, type: 'user', typeLabel: '直售客户退货', fromId: 'L1C', fromName: '华北联合代理',
      ...
      exceptions: [
        { id: 'EX1', time: '2026-07-25 12:05', type: '归属地异常', target: 'RL202607200004', detail: '手机归属广东 · IP地区浙江不一致', notify: '一级+原厂', status: '待处理', dim: 'activate', explainL2: '客户出差导致IP与手机归属不一致，属正常' },
        { id: 'EX2', time: '2026-07-28 11:02', type: 'SN激活异常', target: 'RL202607210001', detail: '跨区激活：IP 浙江 不在直销围栏', notify: '一级+原厂', status: '待处理', dim: 'activate', explainL2: '跨区激活已联系二级核对围栏' },
        { id: 'EX3', time: '2026-08-02 16:10', type: '销售库存异常', target: '广州天河渠道 · 锐涞运动款LL', detail: '本次新增 5 > 预警线（区间销量 × 倍数）', notify: '一级+原厂', status: '待处理', dim: 'stock' },
        { id: 'EX4', time: '2026-07-22 10:40', type: '销售库存异常', target: '宁波海曙店 · 锐涞经典款L', detail: '本次新增 4 > 预警线', notify: '一级+原厂', status: '已处理', dim: 'stock', explain: '已按周转调整进货节奏' },
        { id: 'EX5', time: '2026-08-05 09:20', type: '超量下单预警', target: '杭州城西专营', detail: '2级库存充足仍大量申请下单', notify: '一级+原厂', status: '待处理', dim: 'stock', explain: '' },
        { id: 'EX6', time: '2026-08-06 14:00', type: '扫码尺码不匹配', target: 'RL202608010001', detail: '出货计划 M，实扫 SN 为 L', notify: '原厂', status: '待处理', dim: 'scan' },
        { id: 'EX7', time: '2026-08-07 10:10', type: '客户信息重复', target: 'RL202607200003', detail: '手机号 13800001003 已激活 1 次 · 可查看历史绑定', notify: '一级+原厂', status: '待处理', dim: 'activate', dupPhone: '13800001003', explainL2: '重复手机号为同一家庭成员共用' },
        { id: 'EX8', time: '2026-08-07 15:20', type: '扫码不在库', target: 'RL202607200003', detail: '扫出的 SN 不在本一级仓库（当前 已销售）', notify: '原厂', status: '待处理', dim: 'scan' },
        { id: 'EX9', time: '2026-08-03 14:20', type: '归属地异常', target: 'RL202608030001', detail: '直售激活：手机归属浙江 · IP地区江苏不一致', notify: '一级+原厂', status: '待处理', dim: 'activate', explain: '已核对直销客户手机归属浙江属围栏内' },
        { id: 'EX10', time: '2026-08-07 10:05', type: 'SN激活异常', target: 'RL202608070001', detail: '直售跨区激活：IP 上海 不在直销围栏', notify: '一级+原厂', status: '待处理', dim: 'activate', explain: '客户出差上海激活，一级已确认' },
      ],
      notifications: [
      ...
      customers,
      dailyStats: [],
      seq: { po: 32, so: 104, rt: 13, snBatch: 1, notify: 5, cu: customers.length },
    };
  }
    ...
    try {
      ...
      if (raw) {
        ...
        (parsed.sns || []).forEach((s) => {
          ...
          if (!s.tags) s.tags = [];
          if (s.frozen === undefined) s.frozen = s.status === 'frozen' || s.status === 'factory';
          if (s.status === 'frozen' || s.status === 'factory') {
            s.status = 'warehouse';
            s.frozen = true;
        ...
        (parsed.agentsL1 || []).forEach((a) => {
          if (!a.disableCosign) {
            a.disableCosign = (a.id === 'L1C' && a.status === '启用')
              ? { admin1: true, admin2: false, admin1At: '2026-08-07 09:40', admin1By: 'admin' }
              : { admin1: false, admin2: false };
          }
        });
        ...
        (parsed.purchases || []).forEach((p) => {
          ...
          // 会签完成后立即生效：旧「待生效」迁移为已完成
          if (p.status === 'approvedPending') {
            p.status = 'approved';
            if (!p.approvedAt) p.approvedAt = nowStr();
          }
        });
        parsed.purchases = (parsed.purchases || []).filter((p) => p.status !== 'rejected');
        ...
        (seedDb.returns || []).forEach((row) => {
          const existing = (parsed.returns || []).find((r) => r.id === row.id);
          if (!existing) parsed.returns.push(JSON.parse(JSON.stringify(row)));
        ...
        seed().exceptions.forEach((row) => {
          const existing = parsed.exceptions.find((e) => e.id === row.id);
          if (!existing) parsed.exceptions.push(JSON.parse(JSON.stringify(row)));
          else {
            if (!existing.explain && row.explain) existing.explain = row.explain;
            if (!existing.explainL2 && row.explainL2) existing.explainL2 = row.explainL2;
            if (exceptionDim(row) !== 'stock' && !existing.cosign && row.cosign) {
              existing.cosign = JSON.parse(JSON.stringify(row.cosign));
              if (row.status === '会签中' && isExOpen(existing)) existing.status = '会签中';
            }
          }
        });
        parsed.exceptions.forEach((e) => {
          if (exceptionDim(e) !== 'stock') return;
          if (e.status === '会签中') e.status = '待处理';
          else if (e.status === '已完成') e.status = '已处理';
        });
  ...
  function kitProducts() {
    // 可下单主商品：套件 + 单品（配件无 SN，可单独下单或随套件加购）
  ...
  function canAuditReturn(r) {
    ...
    if (!returnNeedsAudit(r)) return false;
    if (ui.role === 'l2' || ui.role === 'sub') return false;
    return ui.mode === 'admin' && ui.role === 'admin';
  }

  function returnNeedsAudit(r) {
    return !!(r && r.type === 'l1_to_factory');
  }

  function normalizeReturnAuditStatus(r) {
    if (!r || returnNeedsAudit(r)) return;
    if (r.status === 'pending' || r.status === 'approved') r.status = 'done';
  }
  ...
  function returnCustomerInfo(r) {
    const fromRec = r?.customer;
    if (fromRec && (fromRec.name || fromRec.phone || fromRec.phoneLoc || fromRec.region || fromRec.addr)) {
      return {
        name: fromRec.name || '—',
        phone: fromRec.phone || '—',
        region: fromRec.phoneLoc || fromRec.region || fromRec.addr || '—',
      };
    }
    const users = (r?.sns || [])
      .map((sn) => db.sns.find((s) => s.sn === sn))
      .map((row) => row?.user || row?.prevUser)
      .filter(Boolean);
    const u = users[0];
    if (!u) return { name: '—', phone: '—', region: '—' };
    return {
      name: u.name || '—',
      phone: u.phone || '—',
      region: u.phoneLoc || u.region || u.addr || '—',
    };
  }

  function l1ReturnTypeTab(r) {
    if (r.type === 'l1_to_factory') return 'factory';
    const fromIsL1 = db.agentsL1.some((a) => a.id === r.fromId);
    if (fromIsL1 && (r.type === 'user' || r.type === 'self')) return 'user';
    return 'l2';
  }
  ...
  function exceptionDim(e) {
    ...
    if (/扫码|尺码不匹配|不在库/.test(e.type || '')) return 'scan';
    return 'activate';
  }
  function exceptionAgentInfo(e) {
    const sn = db.sns.find((s) => s.sn === e.target);
    if (sn) {
      return {
        l1Id: sn.l1Id || '',
        l2Id: sn.l2Id || '',
        l1Name: sn.l1Id ? l1Name(sn.l1Id) : '—',
        l2Name: sn.l2Id ? l2Name(sn.l2Id) : '—',
      };
    }
    if (e.l2Id) {
      const l2 = db.agentsL2.find((a) => a.id === e.l2Id);
      return {
        l1Id: l2?.parentId || '',
        l2Id: e.l2Id,
        l1Name: l1Name(l2?.parentId),
        l2Name: l2Name(e.l2Id),
      };
    }
    const l2 = db.agentsL2.find((a) => String(e.target || '').includes(a.name) || String(e.detail || '').includes(a.name));
    if (l2) {
      return { l1Id: l2.parentId || '', l2Id: l2.id, l1Name: l1Name(l2.parentId), l2Name: l2.name };
    }
    const l1 = db.agentsL1.find((a) => String(e.target || '').includes(a.name) || String(e.detail || '').includes(a.name));
    if (l1) return { l1Id: l1.id, l2Id: '', l1Name: l1.name, l2Name: '—' };
    return { l1Id: '', l2Id: '', l1Name: '—', l2Name: '—' };
  }
  function exceptionTab(e) {
    const dim = exceptionDim(e);
    if (dim === 'stock') return 'stock';
    if (dim === 'scan') return 'scan';
    return exceptionAgentInfo(e).l2Id ? 'activate-dist' : 'activate-direct';
  }
  function exceptionPageTab() {
    const t = ui.tabs.exception;
    if (!t || t === 'scan' || t === 'activate') return 'activate-direct';
    return t;
  }
  function exceptionTabLabel(tab) {
    return ({
  ...
  function exceptionExplainLabel(tab) {
    return tab === 'activate-dist' ? '二级解释' : '一级解释';
  }
  function exceptionExplainText(e, tab) {
    if ((tab || exceptionTab(e)) === 'activate-dist') return e.explainL2 || e.explain || '';
    return e.explain || '';
  }
  function clip8(s) {
  ...
  function isExOpen(e) {
    if (!e) return false;
    if (exceptionDim(e) === 'stock') return e.status === '待处理';
    return e.status === '待处理' || e.status === '会签中';
  }
  function normalizeExStatus(e) {
    if (!e) return;
    if (e.status === '未处理' || e.status === 'pending') e.status = '待处理';
    else if (e.status === '会签中' || e.status === 'cosigning') e.status = '会签中';
    else if (e.status === '已处理' || e.status === '仅记录' || e.status === '已关闭' || e.status === '已完成') e.status = '已完成';
    if (exceptionDim(e) === 'stock') {
      if (e.status === '会签中') e.status = '待处理';
      else if (e.status === '已完成') e.status = '已处理';
    }
  }
  function displayExStatus(e) {
    if (!e) return '—';
    if (exceptionDim(e) === 'stock') {
      if (e.status === '已处理' || e.status === '已完成') return '已处理';
      return '待处理';
    }
    return e.status || '—';
  }
  function exClosedStatus(e) {
    return exceptionDim(e) === 'stock' ? '已处理' : '已完成';
  }
  function exStatusTone(st) {
    if (st === '待处理') return 'orange';
    if (st === '会签中') return 'blue';
    return 'green';
  }
  function exceptionCustomer(e) {
    const sn = db.sns.find((s) => s.sn === e?.target);
    const u = sn?.user || sn?.prevUser;
    if (!u) return null;
    return { name: u.name || '', gender: u.gender || '', phone: u.phone || '' };
  }
  ...
  function reportScanNotInStock(sn, row) {
    const exists = (db.exceptions || []).some((e) => e.type === '扫码不在库' && e.target === sn && isExOpen(e));
    if (exists) return;
    const who = ui.role === 'l2' ? '二级' : '一级';
    const detail = row
      ? `扫出的 SN 不在本${who}仓库（当前 ${snStatusMeta(row).label}）`
      : `扫出的 SN 不在码库 / 本${who}仓库`;
    pushException('扫码不在库', sn, detail, 'scan');
  }

  function snInOwnWarehouse(row) {
    if (!row || row.frozen || (row.tags || []).includes('已冻结')) return false;
    if (ui.role === 'l2') return row.status === 'l2' && row.l2Id === currentL2Id();
  ...
  function soScanPlanHtml(s) {
    ...
    <div class="page-card scan-sn-list" style="margin-top:10px">
      <div class="table-caption">已扫 SN（${scanned.length}）</div>
      <div class="scan-sn-chips">${scanned.map((sn) => tag(sn, 'green')).join(' ') || emptyHint('尚未扫描')}</div>
    </div>`;
  }
  ...
  function tryAddSnToSale(s, sn) {
    ...
    const row = db.sns.find((x) => x.sn === sn);
    if (!row) {
      reportScanNotInStock(sn, null);
      return { ok: false, msg: `SN 不存在：${sn}` };
    }
    if (row.frozen || (row.tags || []).includes('已冻结')) return { ok: false, msg: `已冻结 SN 不可出货：${sn}` };
    if (row.l1Id !== s.l1Id || row.status !== 'l1') {
      ...
      ? sns.map((sn) => {
        ...
        if (!s) {
          return `<tr>
            ...
            <td colspan="4"><span class="muted">未找到该 SN</span></td>
          </tr>`;
        }
        const st = snStatusMeta(s);
        const tags = snBusinessTags(s);
        return `<tr>
          <td class="num">${escapeHtml(sn)}</td>
          <td>${escapeHtml(productName(s.productId))}</td>
          <td>${escapeHtml(stockSpecText(s.size, s.belt))}</td>
          <td>${tag(st.label, st.tone)}</td>
          <td>${tags.map((t) => tag(t, 'orange')).join(' ') || '—'}</td>
        </tr>`;
      }).join('')
      : `<tr><td colspan="5" class="muted" style="text-align:center">填写下方 SN 后自动带出商品名称、规格等详情</td></tr>`;
    const returnedN = sns.filter((sn) => {
      const s = db.sns.find((x) => x.sn === sn);
      return s && snBusinessTags(s).includes('已退货');
    }).length;
    return `<div class="page-card table-wrap" id="return-sn-detail" style="margin-bottom:10px">
      <div class="table-caption">商品详情${sns.length ? `（${sns.length}）` : ''}${sns.length ? ` · 其中 ${returnedN} 个带「已退货」标签` : ''}</div>
      <table class="data">
        <thead><tr><th>SN</th><th>商品名称</th><th>规格</th><th>状态</th><th>标签</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
  }
  ...
  function pushException(type, target, detail, dim, opts = {}) {
    ...
    // off=异常不报警：只落库，默认已完成/已处理，不推送待处理预警
    const status = (mode === 'off' || mode === 'soft')
      ? (d === 'stock' ? '已处理' : '已完成')
      : '待处理';
    const row = {
  ...
  function signExceptionProcess(e) {
    if (!e) return false;
    if (exceptionDim(e) === 'stock') {
      if (e.status === '已处理' || e.status === '已完成') return false;
      e.status = '已处理';
      toast('已处理该销售库存异常');
      addLog(`处理异常 ${e.type} · 已处理`, 'exception');
      return true;
    }
    if (e.status === '已完成' || e.status === '已处理') return false;
    e.cosign = e.cosign || { admin1: false, admin2: false };
    const slot = adminCosignSlot();
    if (e.cosign[slot]) {
      toast('当前账号已会签，等待另一管理员确认', 'err');
      return false;
    }
    e.cosign[slot] = true;
    if (e.cosign.admin1 && e.cosign.admin2) {
          e.status = exClosedStatus(e);
      toast('会签完成，异常已标记为已完成');
    } else {
      e.status = '会签中';
      toast('已提交会签，待另一管理员确认');
    }
    addLog(`处理异常 ${e.type} · ${e.status}`, 'exception');
    return true;
  }
  ...
  function snCanonicalStatus(row) {
    if (!row) return '';
    if (row.status === 'frozen' || row.status === 'factory') return 'warehouse';
    return SN_STATUS_LABEL[row.status] ? row.status : (row.status || '');
  }

  function addSnTag(row, name) {
    if (!row || !name) return;
    row.tags = [...new Set([...(row.tags || []), name])];
  }
  ...
  function snStatusMeta(row) {
    ...
    const map = {
      ...
      l2: { label: '二级在库', tone: 'blue' },
      bound: { label: '已销售', tone: 'orange' },
    };
    const st = snCanonicalStatus(row);
    return map[st] || { label: st || '未知', tone: 'gray' };
  }

  function snBusinessTags(row) {
    if (!row) return [];
    const raw = row.tags || [];
    const tags = [];
    if (raw.includes('已退货') || row.reIn) tags.push('已退货');
    if (raw.includes('再入库')) tags.push('再入库');
    if (raw.includes('再销售') || row.resale) tags.push('再销售');
    if (raw.includes('已冻结') || row.frozen || row.status === 'frozen' || row.status === 'factory') tags.push('已冻结');
    return SN_BIZ_TAGS.filter((t) => tags.includes(t));
  }

  function snDisplayTags(row) {
    const biz = snBusinessTags(row);
    const extra = (row.tags || []).filter((t) => !SN_BIZ_TAGS.includes(t) && !['冷冻', '可再销售'].includes(t));
    return [...biz, ...extra];
  }

  function snHistoryTags(row) {
    return snDisplayTags(row);
  }
  ...
  function getSnLifecycle(row) {
    ...
    if (row.frozen || (row.tags || []).includes('已冻结')) {
      ev.push({ time: row.returnAt || row.factoryAt || nowStr(), title: '一级退回原厂', desc: '状态改为原厂在库，标签已冻结', type: 'factory' });
    }
  ...
  function pendingReturnCount() {
    return db.returns.filter((r) => r.status === 'pending' && returnNeedsAudit(r)).length;
  }
  function openExCount() {
    return db.exceptions.filter((e) => isExOpen(e) && exceptionDim(e) !== 'scan').length;
  }

  function getOpenExceptionIdsInFilter() {
    const tab = exceptionPageTab();
    const f = ui.filters.exception || {};
    return db.exceptions.filter((e) => {
      if (!isExOpen(e)) return false;
      if (exceptionTab(e) !== tab) return false;
      if (f.status && displayExStatus(e) !== f.status) return false;
      if (!matchExceptionType(e, f.type)) return false;
      if (f.from || f.to) {
        if (!inDateRange(e.time, f.from, f.to)) return false;
      }
      const info = exceptionAgentInfo(e);
      if (f.l1 && info.l1Id !== f.l1) return false;
      if (tab !== 'activate-direct' && f.l2 && info.l2Id !== f.l2) return false;
      return true;
    }).map((e) => e.id);
  }
  ...
  function warnMultipliersInScope(f) {
    const vals = [];
    const add = (a) => {
      if (!a) return;
      const n = Number(a.warnMultiplier || DEFAULT_WARN_MULT);
      if (n) vals.push(n);
    };
    if (f?.l2) add(db.agentsL2.find((x) => x.id === f.l2));
    else if (f?.l1) {
      add(db.agentsL1.find((x) => x.id === f.l1));
      db.agentsL2.filter((a) => a.parentId === f.l1 && !a.pending).forEach(add);
    } else {
      (db.agentsL1 || []).forEach(add);
      (db.agentsL2 || []).filter((a) => !a.pending).forEach(add);
    }
    return [...new Set(vals)];
  }
  function currentAgentWarnMultiplier(f) {
    const uniq = warnMultipliersInScope(f);
    if (uniq.length !== 1) return '-';
    return uniq[0];
  }
  function sortWarnExThenNewest(rows, metaOf) {
    return rows.slice().sort((a, b) => {
      const ao = metaOf(a)?.open ? 0 : 1;
      const bo = metaOf(b)?.open ? 0 : 1;
  ...
  function l1ExCount(l1Id) {
    return db.exceptions.filter((e) => {
      if (!isExOpen(e) || exceptionDim(e) === 'scan') return false;
      return exceptionAgentInfo(e).l1Id === l1Id;
    }).length;
  }
  ...
  function l1PendingAftersaleCount(l1Id) {
    ...
    return db.returns.filter((r) => {
      if (r.status !== 'pending' || !returnNeedsAudit(r)) return false;
  ...
  function adminCosignSlot() {
    return ui.account === 'admin2' ? 'admin2' : 'admin1';
  }
  function l1DisableCosign(a) {
    return a?.disableCosign || { admin1: false, admin2: false };
  }
  function l1DisablePending(a) {
    if (!a || a.status === '停用') return false;
    const c = l1DisableCosign(a);
    return !!(c.admin1 || c.admin2) && !(c.admin1 && c.admin2);
  }
  function pendingL1DisableForAccount(account) {
    const slot = account === 'admin2' ? 'admin2' : 'admin1';
    return db.agentsL1.filter((a) => l1DisablePending(a) && !l1DisableCosign(a)[slot]);
  }
  function l1StatusHtml(a) {
    if (a.status === '停用') return tag('停用', 'gray');
    if (l1DisablePending(a)) return `${tag('启用', 'green')} ${tag('停用待会签', 'orange')}`;
    return tag(a.status, a.status === '启用' ? 'green' : 'gray');
  }
  function applyL1Disable(a) {
    a.status = '停用';
    a.disableCosign = { admin1: false, admin2: false };
    ...
    addLog(`停用一级 ${a.name}（双人会签完成）`);
  }
  function signL1Disable(a) {
    const slot = adminCosignSlot();
    const c = { admin1: false, admin2: false, ...l1DisableCosign(a) };
    if (c[slot]) {
      toast('你已签字，等待另一管理员确认');
      return 'wait';
    }
    c[slot] = true;
    c[`${slot}At`] = nowStr();
    c[`${slot}By`] = ui.account;
    a.disableCosign = c;
    if (c.admin1 && c.admin2) {
      applyL1Disable(a);
      toast('双人会签完成，一级代理已停用');
      return 'done';
    }
    addLog(`一级停用会签 ${a.name} by ${ui.account}`);
    pushNotify('一级停用待会签', `${a.name} 待另一管理员确认停用（当前仍可正常使用）`, '原厂');
    toast('已签字。仅一名管理员确认时，一级代理仍可正常使用');
    return 'signed';
  }
  function l1DisableBtnHtml(a) {
    if (a.status === '停用') {
      return `<button class="btn btn-primary" data-action="disable-l1" data-id="${a.id}">启用</button>`;
    }
    const slot = adminCosignSlot();
    if (l1DisablePending(a) && l1DisableCosign(a)[slot]) {
      return '<button class="btn" disabled>停用（已签字，待会签）</button>';
    }
    const label = l1DisablePending(a) ? '确认停用会签' : '停用';
    return `<button class="btn btn-danger" data-action="disable-l1" data-id="${a.id}">${label}</button>`;
  }
  function l2DisableCosign(a) {
    return a?.disableCosign || { admin1: false, admin2: false };
  }
  function l2DisablePending(a) {
    if (!a || a.status === '停用') return false;
    const c = l2DisableCosign(a);
    return !!(c.admin1 || c.admin2) && !(c.admin1 && c.admin2);
  }
  function pendingL2DisableForAccount(account) {
    const slot = account === 'admin2' ? 'admin2' : 'admin1';
    return db.agentsL2.filter((a) => !a.pending && l2DisablePending(a) && !l2DisableCosign(a)[slot]);
  }
  function l2StatusHtml(a) {
    if (a.status === '停用') return tag('停用', 'gray');
    if (l2DisablePending(a)) return `${tag('启用', 'green')} ${tag('停用待会签', 'orange')}`;
    return tag(a.status, a.status === '启用' ? 'green' : 'gray');
  }
  function applyL2Disable(a) {
    a.status = '停用';
    a.disableCosign = { admin1: false, admin2: false };
    addLog(`停用二级 ${a.name}（双人会签完成）`);
  }
  function signL2Disable(a) {
    const slot = adminCosignSlot();
    const c = { admin1: false, admin2: false, ...l2DisableCosign(a) };
    if (c[slot]) {
      toast('你已签字，等待另一管理员确认');
      return 'wait';
    }
    ...
    if (c.admin1 && c.admin2) {
      applyL2Disable(a);
      toast('双人会签完成，二级代理已停用');
      return 'done';
    }
    addLog(`二级停用会签 ${a.name} by ${ui.account}`);
    pushNotify('二级停用待会签', `${a.name} 待另一管理员确认停用（当前仍可正常使用）`, '原厂');
    toast('已签字。仅一名管理员确认时，二级代理仍可正常使用');
    return 'signed';
  }
  function l2DisableBtnHtml(a) {
    if (a.status === '停用') {
      return `<button class="btn btn-primary" data-action="disable-l2" data-id="${a.id}">启用</button>`;
    }
    const slot = adminCosignSlot();
    if (l2DisablePending(a) && l2DisableCosign(a)[slot]) {
      return '<button class="btn" disabled>停用（已签字，待会签）</button>';
    }
    const label = l2DisablePending(a) ? '确认停用会签' : '停用';
    return `<button class="btn btn-danger" data-action="disable-l2" data-id="${a.id}">${label}</button>`;
  }
  function maybePromptL1Disable() {
    if (!ui.loggedIn || ui.mode !== 'admin') return false;
    if (ui.modal || ui.confirm) return false;
    if (ui._l1DisablePrompted) return false;
    const l1s = pendingL1DisableForAccount(ui.account);
    const l2s = pendingL2DisableForAccount(ui.account);
    if (!l1s.length && !l2s.length) return false;
    ui._l1DisablePrompted = true;
    openModal('l1-disable-pending', { ids: l1s.map((a) => a.id), l2Ids: l2s.map((a) => a.id) });
    return true;
  }
  ...
  function l2ExCount(l2Id) {
    return db.exceptions.filter((e) => {
      if (!isExOpen(e) || exceptionDim(e) === 'scan') return false;
      return exceptionAgentInfo(e).l2Id === l2Id;
    }).length;
  }
  ...
  function syncL1Draft() {
    ...
    if ($('#f-city-q')) {
      ...
      ui.modal.draft.directCityQ = ui.modal.draft.cityQ;
    }
  ...
  function segmentsMatch(p) {
    const need = purchaseNeedQty(p);
    const got = purchaseSegCount(p);
    if (need === 0) return !!(p.parts || []).some((x) => Number(x.qty) > 0) && got === 0;
    return got === need;
  }
  function segmentCount(seg) {
    if (!String(seg || '').trim()) return 0;
    const list = parseSegment(seg);
  ...
  function backToL1DetailAction(fallbackGo, fallbackLabel = '返回列表') {
    if (ui.backRoute === 'agent-l1-detail') {
      return '<button class="btn" data-go="agent-l1-detail">返回</button>';
    }
    if (ui.backRoute === 'agent-l2-detail') {
      return '<button class="btn" data-go="agent-l2-detail">返回</button>';
    }
    return fallbackGo ? `<button class="btn" data-go="${fallbackGo}">${escapeHtml(fallbackLabel)}</button>` : '';
  }
  function sidebarActiveId() {
    const r = ui.route;
    if (r === 'agent-l1-detail' || r === 'l1-return-detail') return 'agent-l1';
    if (r === 'agent-l2-detail' || r === 'l2-return-detail') return 'agent-l2';
    return r;
  }
  function metricCard(label, value, go, setFilter = '', tone = '') {
    const attr = go ? ` data-go="${go}"${setFilter ? ` data-set-filter="${setFilter}"` : ''} style="cursor:pointer"` : '';
    const cls = tone ? ` metric-card--${tone}` : '';
    return `<div class="metric-card${cls}"${attr}><div class="metric-label">${escapeHtml(label)}</div><div class="metric-value num">${value}</div></div>`;
  }
  ...
  function tabsHtml(key, items) {
    ...
    return `<div class="page-card"><div class="tabs">${items.map((it) => {
      const cls = it.badgeTone === 'ok' ? 'menu-badge menu-badge-ok' : 'menu-badge';
      const badge = it.badge != null ? `<span class="${cls}">${it.badge}</span>` : '';
  ...
  function exceptionTypeOptions(tab) {
    const seen = [];
    db.exceptions.forEach((e) => {
      if (exceptionTab(e) !== tab) return;
      const t = e.type || '';
  ...
  function miniSegHtml(key, items) {
    ...
    return `<div class="mini-seg" role="tablist">${items.map((it) => {
      const cls = it.badgeTone === 'ok' ? 'mini-seg-badge mini-seg-badge-ok' : 'mini-seg-badge';
      return `
      <button type="button" class="mini-seg-btn ${cur === it.id ? 'on' : ''}" data-tab="${key}:${it.id}" role="tab" aria-selected="${cur === it.id}">
        ${escapeHtml(it.title)}${it.badge != null ? `<span class="${cls}">${it.badge}</span>` : ''}
      </button>`;
    }).join('')}</div>`;
  }
  ...
  function pageHome() {
    ...
    return `${pageHeader('工作台', '平台运营总览与待办')}
      <div class="metric-grid">
        ${metricCard('一级代理', db.agentsL1.filter((a) => a.status === '启用').length, 'agent-l1', '', 'info')}
        ${metricCard('二级代理', db.agentsL2.filter((a) => a.auditStatus === 'approved' && !a.pending).length, 'agent-l2', '', 'dist')}
        ${metricCard('筛选区间销量', monthSales, 'sales', '', 'range')}
        ${metricCard('已销售SN', snBound, 'sn', '', 'hist')}
        ${metricCard('待处理采购', pendingPo, 'purchase', '', 'pending')}
        ${metricCard('待处理异常', openEx, 'exception', '', 'warn')}
      </div>
      <div class="page-card" style="margin-top:12px">
        ...
        <div>
          ...
          <button class="todo-row" data-go="purchase"><span>采购待处理/会签</span><span class="todo-count">${pendingPo}</span></button>
          <button class="todo-row" data-go="exception"><span>待处理异常</span><span class="todo-count ${openEx ? 'hot' : ''}">${openEx}</span></button>
  ...
  function pageAgentL1() {
    ...
    rows = sortAgentsExFirst(rows, (a) => l1ExCount(a.id));
    const tr = rows.map((a) => {
        ...
        <td>
          <div class="status-stack">
            ...
            <div class="status-metrics">
              ...
              <span class="${poN ? 'hot' : ''}">采购待处理 ${poN}</span>
  ...
  function l1DetailJumpAttr(a) {
    return {
      ...
      ex: `data-go="exception" data-set-filter="exception:l1=${a.id};exception:l2=;exception:type=" data-set-tab="exception:activate-direct" data-back="agent-l1-detail"`,
      ...
      stock: `data-go="stock" data-set-filter="stock:type=l1;stock:agent=${a.id}" data-set-tab="stock:summary" data-back="agent-l1-detail"`,
    };
  }
  function l2DetailJumpAttr(a) {
    return {
      ...
      ex: `data-go="exception" data-set-filter="exception:l1=${a.parentId || ''};exception:l2=${a.id};exception:type=" data-set-tab="exception:activate-dist" data-back="agent-l2-detail"`,
      ...
      stock: `data-go="stock" data-set-filter="stock:type=l2;stock:agent=${a.id}" data-set-tab="stock:summary" data-back="agent-l2-detail"`,
    };
  }

  function pageAgentL1Detail() {
    const f = ui.filters['agent-l1-detail'] || {};
    const a = db.agentsL1.find((x) => x.id === f.id);
    const backList = '<button class="btn" data-go="agent-l1">返回列表</button>';
    if (!a) {
      return `${pageHeader('一级代理详情', '未找到该一级代理', backList)}<div class="page-card">${emptyHint('请从一级代理商列表点击进入')}</div>`;
    }
    const acc = l1Account(a.id);
    const exN = l1ExCount(a.id);
    const poN = l1PendingPoCount(a.id);
    const rtN = l1PendingAftersaleCount(a.id);
    const jump = l1DetailJumpAttr(a);
    return `${pageHeader('一级代理详情', a.name, backList)}
      <div class="page-card">
        <div class="detail-grid">
          ...
          <div><span>状态</span>${l1StatusHtml(a)}</div>
          <div><span>预警倍数</span>${a.warnMultiplier||1.5}</div>
          <div><span>报警粒度</span>${tag(a.warnMode==='soft'?'软报警':'严格', a.warnMode==='soft'?'gray':'orange')}</div>
          <div><span>本月采购量</span><strong class="num">${l1MonthPurchaseQty(a.id)}</strong></div>
          <div><span>本月销售量</span><strong class="num">${l1MonthSalesQty(a.id)}</strong></div>
          <div><span>当前库存总数</span><strong class="num">${l1StockCount(a.id)}</strong></div>
        </div>
      ...
      <div class="page-card" style="margin-top:12px">
        <div class="page-actions">
          ...
          <button class="btn btn-primary" ${jump.purchase}>采购</button>
          <button class="btn btn-primary" ${jump.sales}>销售</button>
          <button class="btn btn-primary" ${jump.ret}>退货</button>
          <button class="btn btn-primary" ${jump.stock}>库存</button>
          <button class="btn ${exN ? 'btn-danger' : ''}" ${jump.ex}>异常${exN ? ` ${exN}` : ''}</button>
          <button class="btn" ${jump.poPending}>采购待处理${poN ? ` ${poN}` : ''}</button>
          <button class="btn" ${jump.aftersale}>售后待处理${rtN ? ` ${rtN}` : ''}</button>
          ${l1DisableBtnHtml(a)}
        </div>
      </div>`;
  }

  function pageAgentL2Detail() {
    const f = ui.filters['agent-l2-detail'] || {};
    const a = db.agentsL2.find((x) => x.id === f.id);
    const backList = '<button class="btn" data-go="agent-l2">返回列表</button>';
    if (!a) {
      return `${pageHeader('二级代理详情', '未找到该二级代理', backList)}<div class="page-card">${emptyHint('请从二级代理商列表点击进入')}</div>`;
    }
  ...
  function pageL1ReturnDetail() {
    ...
    if (!ui.tabs['l1-return']) ui.tabs['l1-return'] = 'all';
    const tab = ui.tabs['l1-return'] || 'all';
    const from = f.from || monthStart();
    const to = f.to || todayDate();
    const l2Opts = db.agentsL2.filter((a) => !a.pending && a.parentId === l1Id);
    const l2Id = l2Opts.some((a) => a.id === f.l2) ? f.l2 : '';
    ...
    const countOf = (id) => list.filter((r) => l1ReturnTypeTab(r) === id).length;
    if (tab && tab !== 'all') list = list.filter((r) => l1ReturnTypeTab(r) === tab);
    if (tab !== 'user' && l2Id) {
    ...
    return `${pageHeader('一级退货详情', `${l1Name(l1Id)} · 可改筛选区间`, backToL1DetailAction('agent-l1'))}
      ${tabsHtml('l1-return', returnKindTabItems(countOf, all.filter((r) => inDateRange(r.createdAt, from, to)).length))}
      ${filterBar(`
      ...
      `)}
      <div class="metric-grid metric-grid-2">${metricCard('筛选区间退货', monthQty, '', '', 'range')}${metricCard('历史总量', histQty, '', '', 'hist')}</div>
      <div class="page-card table-wrap"><table class="data">
        <thead><tr><th>单号</th><th>类型</th>${showCust ? '<th>姓名</th><th>手机号</th><th>地区</th>' : ''}<th>理由</th><th>商品明细</th>${noAudit ? '' : thFilterHtml('状态', 'l1-return', 'status', RT_STATUS_OPTS)}<th>时间</th></tr></thead>
        <tbody>${list.map((r)=>{
          const cust = returnCustomerInfo(r);
          ...
          ${noAudit ? '' : `<td>${tag(r.status==='pending'?'待审':r.status==='approved'?'已通过':r.status)}</td>`}
          <td>${escapeHtml(r.createdAt)}</td>
        </tr>`;
        }).join('') || `<tr><td colspan="${(showCust ? 8 : 5) + (noAudit ? 0 : 1)}">${emptyHint()}</td></tr>`}</tbody>
      </table></div>`;
  }
  ...
  function pageAgentL2() {
    ...
    if (sortKey === 'parent' || sortKey === 'name') {
      rows.sort((a, b) => {
        let av, bv;
  ...
  function pageProduct() {
    ...
    return `${pageHeader('商品库', '套件 / 单品（有SN）；套件可勾选随售单品',
      ...
      `)}
      <div class="page-card table-wrap"><table class="data">
        <thead><tr><th>编码</th><th>名称</th><th>类型</th><th>组件/规格</th><th>标品组合</th><th>操作</th></tr></thead>
        <tbody>${rows.map((p)=>{
        ...
        }).join('') || `<tr><td colspan="6">${emptyHint('暂无商品')}</td></tr>`}</tbody>
      </table></div>`;
  }
  ...
  function pageSN() {
    ...
    if (f.status) rows = rows.filter((s) => snCanonicalStatus(s) === f.status);
    if (f.tag) rows = rows.filter((s) => snHistoryTags(s).includes(f.tag));
  ...
  function pagePurchase() {
    if (ui.tabs.purchase === 'rejected') ui.tabs.purchase = 'all';
    const tab = ui.tabs.purchase || 'all';
    const f = applyListDates(ui.filters.purchase || (ui.filters.purchase = {}));
    const livePos = db.purchases.filter((p) => p.status !== 'rejected');
    let rows = livePos.slice();
    ...
    rows = sortWarnExThenNewest(rows, (p) => stockWarnExMeta(p.l1Id, ''));
    const poScope = livePos.filter((p) => !f.l1 || p.l1Id === f.l1);
    ...
    const poN = (st) => livePos.filter((p) => p.status === st).length;
    const tabItems = [
      { id: 'all', title: '全部', badge: livePos.length || null },
    ...
    return `${pageHeader('采购单管理', '一站式审核：标准/非标/配件 + 段号起止 + 双人会签即完成（下单仅一级小程序）', backToL1DetailAction())}
  ...
  function pageSales() {
    const f = applyListDates(ui.filters.sales || (ui.filters.sales = {}));
    ...
    const rows = sortWarnExThenNewest(
      scoped.filter((s) => inDateRange(s.createdAt, f.from, f.to)),
      (s) => stockWarnExMeta(s.l1Id, s.channel === 'direct' ? '' : s.l2Id),
    );
    ...
    return `${pageHeader('销售单管理', desc, backToL1DetailAction())}
      ...
      `)}
      <div class="metric-grid metric-grid-2">${metricCard('筛选区间销量', monthQty, '', '', 'range')}${metricCard('历史销量', histQty, '', '', 'hist')}</div>
      <div class="page-card table-wrap"><table class="data">
  ...
  function pageReturn() {
    ...
    const matchL1 = (r) => {
      ...
      return r.fromId === f.l1 || r.approverId === f.l1 || l2Ids.has(r.fromId)
        || (r.sns || []).some((sn) => db.sns.find((s) => s.sn === sn && s.l1Id === f.l1));
    };
    let byAgent = db.returns.filter(matchL1);
    const inRangeAll = byAgent.filter((r) => inDateRange(r.createdAt, f.from, f.to));
    const countKind = (id) => inRangeAll.filter((r) => l1ReturnTypeTab(r) === id).length;
    let rows = inRangeAll.slice();
    if (kind && kind !== 'all') rows = rows.filter((r) => l1ReturnTypeTab(r) === kind);
    if (kind !== 'user' && f.l2) {
    ...
    rows.sort((a, b) => {
      ...
      return createdStamp(a) - createdStamp(b);
    });
    const monthQty = inRangeAll.reduce((n, r) => n + (r.sns || []).length, 0);
    const histQty = byAgent.reduce((n, r) => n + (r.sns || []).length, 0);
    const rtN = (st) => statusRows.filter((r) => r.status === st).length;
    const l2Select = (kind === 'user' || kind === 'factory') ? '' : `<select class="field-input" data-filter="return:l2"><option value="">全部二级</option>${l2Opts.map((a)=>`<option value="${a.id}" ${f.l2===a.id?'selected':''}>${escapeHtml(a.name)}</option>`).join('')}</select>`;
    const showCust = kind === 'user';
    const statusTabs = noAuditKind ? '' : `${tabsHtml('return', [
    ...
    return `${pageHeader('返货管理', '列表含 SN · 统计可点进详情', `${backToL1DetailAction()}<button class="btn" data-go="stats">数据统计</button>`)}
      ${tabsHtml('return-kind', returnKindTabItems(countKind, inRangeAll.length))}
      ${statusTabs}
      ${filterBar(`
        ...
        <label class="muted">退货时间</label>
        <input type="date" class="field-input" data-filter="return:from" value="${escapeHtml(f.from)}" />
        <input type="date" class="field-input" data-filter="return:to" value="${escapeHtml(f.to)}" />
      `)}
      <div class="metric-grid${noAuditKind ? ' metric-grid-2' : ' metric-grid-3'}">
        ${metricCard('筛选区间退货件数', monthQty, 'return', '', 'range')}
        ${metricCard('历史退货件数', histQty, 'return', '', 'hist')}
        ${pendingCard}
      </div>
      <div class="page-card table-wrap"><table class="data">
  ...
  function pageException() {
    const tab = exceptionPageTab();
    if (ui.tabs.exception !== tab) ui.tabs.exception = tab;
    const f = ui.filters.exception || (ui.filters.exception = {});
    if (tab === 'stock') {
      if (f.status === '会签中') f.status = '待处理';
      if (f.status === '已完成') f.status = '已处理';
    } else if (f.status === '已处理') {
      f.status = '已完成';
    }
    let rows = db.exceptions.filter((e) => exceptionTab(e) === tab);
    if (f.status) rows = rows.filter((e) => displayExStatus(e) === f.status);
    if (f.from || f.to) rows = rows.filter((e) => inDateRange(e.time, f.from, f.to));
    if (f.type) rows = rows.filter((e) => matchExceptionType(e, f.type));
    if (f.l1) rows = rows.filter((e) => exceptionAgentInfo(e).l1Id === f.l1);
    if (tab !== 'activate-direct' && f.l2) rows = rows.filter((e) => exceptionAgentInfo(e).l2Id === f.l2);
    const counts = {
      'activate-direct': db.exceptions.filter((e) => exceptionTab(e) === 'activate-direct' && isExOpen(e)).length,
      'activate-dist': db.exceptions.filter((e) => exceptionTab(e) === 'activate-dist' && isExOpen(e)).length,
      stock: db.exceptions.filter((e) => exceptionTab(e) === 'stock' && isExOpen(e)).length,
    };
    const histTotal = db.exceptions.filter((e) => exceptionTab(e) === tab).length;
    const openN = getOpenExceptionIdsInFilter().length;
    const typeOpts = exceptionTypeOptions(tab);
    const explainLabel = exceptionExplainLabel(tab);
    const statusOpts = tab === 'stock' ? EX_STOCK_STATUS_OPTS : EX_STATUS_OPTS;
    ...
    return `${pageHeader('异常管理', '直售激活 / 分销激活 / 销售库存 · 待处理加粗', `${backToL1DetailAction()}<button class="btn" data-action="open-ex-rules">异常标准配置</button>`)}
      ${tabsHtml('exception', [
        { id: 'activate-direct', title: '直售激活异常', badge: counts['activate-direct'] || null },
        { id: 'activate-dist', title: '分销激活异常', badge: counts['activate-dist'] || null },
      ...
      <div class="page-card table-wrap"><table class="data">
        ...
        <tbody>${rows.map((e)=>{
          const info = exceptionAgentInfo(e);
          const bold = isExOpen(e) ? 'ex-bold' : '';
          const st = displayExStatus(e);
          return `<tr class="${bold} row-clickable" data-row-action="view-exception" data-id="${e.id}">
            ...
            <td>${escapeHtml(info.l1Name)}</td>
            <td>${escapeHtml(tab === 'activate-direct' ? '—' : info.l2Name)}</td>
            <td>${escapeHtml(e.target)}</td><td>${escapeHtml(e.detail)}</td>
            <td title="${escapeHtml(exceptionExplainText(e, tab) || '')}">${escapeHtml(clip8(exceptionExplainText(e, tab)))}</td>
            <td>${tag(st, exStatusTone(st))}</td>
          </tr>`;
        }).join('') || `<tr><td colspan="8">${emptyHint()}</td></tr>`}</tbody>
      </table></div>`;
  }

  function computeStatsSnapshot(date) {
    const monthFrom = String(date).slice(0, 7) + '-01';
    ...
    return {
      date,
      salesMonth: saleQty(soDone, monthFrom, date),
      salesAll: saleQty(soDone, '2020-01-01', date),
      actMonth: db.sns.filter((s) => s.status === 'bound' && inDateRange(s.soldAt || s.bindAt, monthFrom, date)).length,
      actAll: db.sns.filter((s) => s.status === 'bound' && inDateRange(s.soldAt || s.bindAt, '2020-01-01', date)).length,
      stock: db.sns.filter((s) => {
  ...
  function ensureDailyStats() {
    ...
    eachDateStr(monthStart(), today).forEach((d) => {
      const snap = computeStatsSnapshot(d);
      const i = db.dailyStats.findIndex((x) => x.date === d);
      if (i >= 0) {
        if (d === today) db.dailyStats[i] = snap;
      } else {
        db.dailyStats.push(snap);
      }
    });
    db.dailyStats.sort((a, b) => String(a.date).localeCompare(String(b.date)));
    try { saveStore(); } catch (_) {}
  }
  ...
  function pageStats() {
    ...
    const kpi = (label, value, go, filter, setTab, tone) =>
      `<button type="button" class="dash-kpi${tone ? ` dash-kpi--${tone}` : ''}" ${go ? `data-go="${go}"` : ''}${filter ? ` data-set-filter="${filter}"` : ''}${setTab ? ` data-set-tab="${setTab}"` : ''}>
        <span class="dash-kpi-label">${escapeHtml(label)}</span>
        <strong class="dash-kpi-value num">${value}</strong>
      </button>`;
    const poAgent = l1Id ? `purchase:l1=${l1Id}` : 'purchase:l1=';
    const soAgent = `${l1Id ? `sales:l1=${l1Id}` : 'sales:l1='};${l2Id ? `sales:l2=${l2Id}` : 'sales:l2='}`;
    const poRange = `${poAgent};purchase:hist=;purchase:from=${f.from};purchase:to=${f.to}`;
    const poAll = `${poAgent};purchase:hist=1;purchase:from=;purchase:to=`;
    const soRange = `${soAgent};sales:hist=;sales:from=${f.from};sales:to=${f.to}`;
    const soAll = `${soAgent};sales:hist=1;sales:from=;sales:to=`;
    const stockFilter = l2Id
      ? `stock:type=l2;stock:agent=${l2Id}`
      : (l1Id ? `stock:type=l1;stock:agent=${l1Id}` : 'stock:type=;stock:agent=');
    ...
    return `${pageHeader('数据统计', '采购 / 销售 / 在库 · 可按一二级与日期筛选')}
      ...
      <div class="dash">
        <div class="dash-kpis dash-kpis-7">
          ${kpi('区间采购数', st.purchaseRange, 'purchase', poRange, 'purchase:all', 'po')}
          ${kpi('累计采购数', st.purchaseAll, 'purchase', poAll, 'purchase:all', 'po-all')}
          ${kpi('区间直售数', st.directRange, 'sales', soRange, 'sales:direct', 'direct')}
          ${kpi('累计直售数', st.directAll, 'sales', soAll, 'sales:direct', 'direct-all')}
          ${kpi('区间分销数', st.distRange, 'sales', soRange, 'sales:distribute', 'dist')}
          ${kpi('累计分销数', st.distAll, 'sales', soAll, 'sales:distribute', 'dist-all')}
          ${kpi('当前在库数量', st.stock, 'stock', stockFilter, 'stock:summary', 'stock')}
        </div>
        <div class="dash-grid">
  ...
  function pageMiniDirectScan() {
    ...
    return `<div class="alert alert-info">步骤 2/2：填写客户 · SN ${escapeHtml(sn)} · ${escapeHtml(row ? productName(row.productId) + '/' + row.size : '')}</div>
      ...
      <div class="form-field"><label>姓名（选填）</label><input class="field-input" id="direct-name" placeholder="客户姓名" value="${escapeHtml(ui.form.directName || '')}" /></div>
      <div class="form-field"><label>性别（选填）</label>
        <select class="field-input" id="direct-gender">
          <option value="">未填写</option>
          <option value="男" ${ui.form.directGender==='男'?'selected':''}>男</option>
          <option value="女" ${ui.form.directGender==='女'?'selected':''}>女</option>
        </select>
      </div>
      <div class="form-field"><label>年龄（选填）</label><input class="field-input" id="direct-age" type="number" min="0" max="120" placeholder="岁" value="${escapeHtml(ui.form.directAge || '')}" /></div>
      <div class="form-field"><label>客户情况说明（选填）</label><textarea class="field-input" id="direct-note" rows="2" placeholder="可手写补充">${escapeHtml(ui.form.directNote || '')}</textarea></div>
      <p class="mini-page-desc">校验：IP 须在直销围栏内；手机归属地须与 IP 地区一致。姓名/性别/年龄/情况说明为非必填。</p>
  ...
  function miniPurchaseBody() {
    ...
    list = sortWarnExThenNewest(list, (p) => stockWarnExMeta(p.l1Id, ''));
  ...
  function miniSalesBody() {
    ...
    list = sortWarnExThenNewest(list, (s) => stockWarnExMeta(s.l1Id, s.channel === 'direct' ? '' : s.l2Id));
  ...
  function miniAftersaleBody() {
    ...
    const typeTab = ['upward', 'cend', 'l2_apply'].includes(ui.tabs.miniRtType) ? ui.tabs.miniRtType : 'l2_apply';
    if (ui.role === 'l1' && ui.tabs.miniRtType !== typeTab) ui.tabs.miniRtType = typeTab;
    const noAuditMini = ui.role === 'l2' || typeTab === 'l2_apply' || typeTab === 'cend';
    const typeN = (t) => {
      if (t === 'l2_apply') return rtAll.filter((r) => r.type === 'l2_to_l1').length;
      if (t === 'cend') return rtAll.filter((r) => r.type === 'user').length;
    ...
    if (ui.role === 'l1') {
      ...
      else if (typeTab === 'cend') list = list.filter((r) => r.type === 'user');
      else list = list.filter((r) => r.type === 'l2_to_l1');
    }
    if (!noAuditMini && statusTab !== 'all') list = list.filter((r) => r.status === statusTab);
    ...
    return `${ui.role==='l2'
        ? `<button class="btn btn-block btn-primary" data-action="mini-create-cend-return" style="margin-bottom:10px">填写终端退货单</button>`
        : `<button class="btn btn-block" data-action="mini-create-return" style="margin-bottom:8px">填写二级退一级</button>
           <button class="btn btn-block btn-primary" data-action="mini-create-cend-return" style="margin-bottom:8px">填写终端退货单</button>
           <button class="btn btn-block" data-action="mini-create-factory-return" style="margin-bottom:10px">申请退原厂</button>`}
      ${ui.role === 'l1' ? miniSegHtml('miniRtType', [
        { id: 'l2_apply', title: '二级退一级', badge: typeN('l2_apply') || null },
        { id: 'cend', title: '终端退货', badge: typeN('cend') || null },
        { id: 'upward', title: '退原厂', badge: typeN('upward') || null },
      ]) : ''}
      ${noAuditMini ? '' : miniSegHtml('miniRtStatus', [
        { id: 'all', title: '全部', badge: (ui.role === 'l1'
          ? rtAll.filter((r) => r.type === 'l1_to_factory').length
          : rtAll.length) || null },
        { id: 'pending', title: '待审核', badge: (ui.role === 'l1'
          ? rtAll.filter((r) => r.type === 'l1_to_factory' && r.status === 'pending')
          : rtAll.filter((r) => r.status === 'pending')).length || null },
        { id: 'approved', title: '已通过', badge: (ui.role === 'l1'
          ? rtAll.filter((r) => r.type === 'l1_to_factory' && r.status === 'approved')
          : rtAll.filter((r) => r.status === 'approved')).length || null },
        { id: 'done', title: '已处理', badge: (ui.role === 'l1'
          ? rtAll.filter((r) => r.type === 'l1_to_factory' && r.status === 'done')
          : rtAll.filter((r) => r.status === 'done')).length || null },
      ])}
      ${miniTimeSnFilters('miniRt')}
      <div class="mini-list">${list.map((r)=>`<button type="button" class="mini-list-item ${(!noAuditMini && r.status==='pending')?'rt-pending':''}" data-action="open-view-return" data-id="${r.id}">
        <strong class="rt-row-hd"><span>${escapeHtml(r.no)}</span>${noAuditMini || !returnNeedsAudit(r) ? '' : returnStatusTag(r.status)}</strong>
  ...
  function miniExceptionBody() {
    let tab = ui.tabs.miniEx;
    if (!tab || tab === 'all' || tab === 'scan' || tab === 'activate') {
      tab = ui.role === 'l2' ? 'activate-dist' : 'activate-direct';
      ui.tabs.miniEx = tab;
    }
    let list = db.exceptions.filter((e) => {
      if (exceptionDim(e) === 'scan') return false;
      const info = exceptionAgentInfo(e);
      if (ui.role === 'l2') return info.l2Id === currentL2Id();
      return info.l1Id === currentL1Id();
    });
    const exAll = list.slice();
    const exN = (t) => exAll.filter((e) => exceptionTab(e) === t && isExOpen(e)).length;
    list = list.filter((e) => exceptionTab(e) === tab);
    const ef = ui.filters.miniEx || {};
    list = list.filter((e) => matchTimeSnFilter(e.time, [e.target], ef));
    list = sortByCreated(list);
    return `${miniSegHtml('miniEx', [
      { id: 'activate-direct', title: '直售激活', badge: exN('activate-direct') || null },
      { id: 'activate-dist', title: '分销激活', badge: exN('activate-dist') || null },
      { id: 'stock', title: '销售库存', badge: exN('stock') || null },
    ])}
      ${miniTimeSnFilters('miniEx')}
      <div class="mini-list">${list.map((e)=>`<button type="button" class="mini-list-item ${isExOpen(e)?'ex-bold':''}" data-action="open-view-exception" data-id="${e.id}">
        <strong>${escapeHtml(e.type)}</strong>
        <span>${tag(exceptionTabLabel(exceptionTab(e)))}</span>
        <span>${escapeHtml(e.target)}</span>
        <span>${escapeHtml(e.detail)}</span>
        <span>${tag(displayExStatus(e), exStatusTone(displayExStatus(e)))}</span>
      </button>`).join('') || emptyHint()}</div>`;
  }
  ...
  function pageMiniService() {
    const openExL1 = ui.role === 'l1'
      ? db.exceptions.filter((e) => isExOpen(e) && (
  ...
  function customerAgentIds(c) {
    const l1Ids = new Set();
    const l2Ids = new Set();
    (c.snRows || []).forEach((s) => {
      if (s.l1Id) l1Ids.add(s.l1Id);
  ...
  function customerIsDirect(c) {
    return customerAgentIds(c).l2Ids.length === 0;
  }

  function customerSalesQty(c, from, to) {
    const sns = c.snRows || [];
    const hist = sns.length;
    const range = sns.filter((s) => inDateRange(s.soldAt || s.bindAt || c.updatedAt || c.createdAt, from, to)).length;
    return { range, hist };
  }
  ...
  function customerInDateRange(c, from, to) {
    ...
    (c.snRows || []).forEach((s) => {
      if (s.soldAt) dates.push(s.soldAt);
      if (s.bindAt) dates.push(s.bindAt);
    });
  ...
  function pageCustomers() {
    const f = ui.filters.customers || (ui.filters.customers = {});
    if (!f.from) f.from = monthStart();
    if (!f.to) f.to = todayDate();
    const l2Opts = db.agentsL2.filter((a) => !a.pending && (!f.l1 || a.parentId === f.l1));
    if (f.l2 && !l2Opts.some((a) => a.id === f.l2)) f.l2 = '';
    const directOnly = f.directOnly === '1';
    let rows = listAdminCustomers();
    if (directOnly) rows = rows.filter(customerIsDirect);
    if (f.from || f.to) rows = rows.filter((r) => customerInDateRange(r, f.from, f.to));
    if (f.l1) rows = rows.filter((r) => customerAgentIds(r).l1Ids.includes(f.l1));
    if (!directOnly && f.l2) rows = rows.filter((r) => customerAgentIds(r).l2Ids.includes(f.l2));
    ...
    return `${pageHeader('销售客户', '点击行看详情（编辑 / 删除在详情内）',
      ...
      ${filterBar(`
        <select class="field-input" data-filter="customers:l1"><option value="">全部一级</option>${db.agentsL1.map((a)=>`<option value="${a.id}" ${f.l1===a.id?'selected':''}>${escapeHtml(a.name)}</option>`).join('')}</select>
        ${directOnly ? '' : `<select class="field-input" data-filter="customers:l2"><option value="">全部二级</option>${l2Opts.map((a)=>`<option value="${a.id}" ${f.l2===a.id?'selected':''}>${escapeHtml(a.name)}</option>`).join('')}</select>`}
        <label class="muted">销售时间</label>
        <input type="date" class="field-input" data-filter="customers:from" value="${escapeHtml(f.from)}" />
        <input type="date" class="field-input" data-filter="customers:to" value="${escapeHtml(f.to)}" />
  ...
  const PAGES = {
    home: pageHome, 'agent-l1': pageAgentL1, 'agent-l1-detail': pageAgentL1Detail, 'agent-l2': pageAgentL2, 'agent-l2-detail': pageAgentL2Detail, 'agent-l2-audit': pageAgentL2Audit,
  ...
  function openModal(type, payload = {}) {
    if (type === 'view-agent-l1') {
      ui.filters['agent-l1-detail'] = { ...(ui.filters['agent-l1-detail'] || {}), id: payload.id };
      ui.modal = null;
      navigate('agent-l1-detail');
      return;
    }
    if (type === 'view-agent-l2') {
      ui.filters['agent-l2-detail'] = { ...(ui.filters['agent-l2-detail'] || {}), id: payload.id };
      ui.modal = null;
      navigate('agent-l2-detail');
      return;
    }
  ...
  function citySearchPickerHtml(opts, selected, q, toggleAttr, selectAllAction, hints = {}) {
    const list = opts || [];
    const sel = selected || [];
    const query = q || '';
    const shown = visibleDirectCities(list, sel, query);
    const toggleOpts = query ? filterCitiesByQ(list, query) : list;
    const allOn = isAllSelected(toggleOpts, sel);
    const btnLabel = query ? '全选搜索结果' : (hints.allLabel || '全选');
    return `<div class="form-field span-2"><label>${escapeHtml(hints.label || '城市')} ${selectAllBtn(selectAllAction, btnLabel, allOn)}</label>
      <input class="field-input city-search" id="f-city-q" placeholder="输入城市名搜索后勾选" value="${escapeHtml(query)}" />
      ${shown.length ? chips(shown, sel, toggleAttr) : `<p class="muted" style="margin-top:6px">${list.length ? '无匹配城市' : (hints.empty || '暂无可选城市')}</p>`}
  ...
  function directCityPickerHtml(directOpts, selected, q) {
    return citySearchPickerHtml(directOpts, selected, q, 'data-toggle-direct', 'select-all-direct', {
      label: '直销范围（城市）',
      allLabel: '全选当前可售城市',
      empty: '请先选择可销售范围，再搜索/全选直销城市',
    });
  }
  ...
  function auditLineSpec(line) {
    const name = productName(line.productId);
    const spec = [line.size, line.belt].filter(Boolean).join('+');
    return spec ? `${name} · ${spec}` : name;
  }
  function auditLinesTable(lines, kind, segs) {
    if (!lines.length) return emptyHint(kind === 'lines' ? '无标准行' : '暂无非标，可点击加行');
    const qtySum = lines.reduce((n, l) => n + (Number(l.qty) || 0), 0);
    let segSum = 0;
    const bodies = lines.map((line, idx) => {
      const key = lineSegKey(line);
      const arr = (segs[key] && segs[key].length) ? segs[key] : [''];
      const counts = arr.map((s) => segmentCount(s));
      counts.forEach((c) => { segSum += c; });
      return `<tbody class="audit-line" data-line-kind="${kind}" data-line-idx="${idx}">
        ${arr.map((seg, si) => {
          const bits = String(seg || '').split('-');
          const from = bits[0] || '';
          const to = bits.length > 1 ? bits.slice(1).join('-') : '';
          return `<tr>
            ${si === 0 ? `<td rowspan="${arr.length}">
              <div class="audit-prod">${escapeHtml(auditLineSpec(line))}</div>
              <input type="hidden" data-po-field="productId" value="${escapeHtml(line.productId || '')}" />
              <input type="hidden" data-po-field="size" value="${escapeHtml(line.size || '')}" />
              <input type="hidden" data-po-field="belt" value="${escapeHtml(line.belt || '')}" />
            </td>
            <td rowspan="${arr.length}"><input type="number" class="field-input" data-po-field="qty" value="${line.qty || 0}" /></td>` : ''}
            <td>
              <div class="segment-row" style="display:flex;gap:6px;align-items:center">
            ...
            <td class="num" data-seg-count="${si}">${counts[si]}</td>
          </tr>`;
        }).join('')}
      </tbody>`;
    }).join('');
    return `<div class="page-card table-wrap"><table class="data audit-po-table">
      <thead><tr><th>商品</th><th style="width:88px">数量</th><th>号段</th><th style="width:96px">号段数量</th></tr></thead>
      ${bodies}
      <tfoot><tr>
        <td>总计</td>
        <td class="num" data-audit-qty-sum="${kind}">${qtySum}</td>
        <td class="muted">号段总计</td>
        <td class="num" data-audit-seg-sum="${kind}">${segSum}</td>
      </tr></tfoot>
    </table></div>`;
  }
  ...
  function modalContent() {
    ...
    if (type === 'edit-l1') {
      ...
      body = `<div class="form-grid">
          ...
          ${directCityPickerHtml(directOpts, a.directAreas || [], a.cityQ || a.directCityQ || '')}
      ...
      foot = `<button class="btn" data-action="close-modal">取消</button><button class="btn btn-primary" data-action="save-l1">保存</button>`;
      ...
      body = `<div class="form-grid">
        ...
        ${directCityPickerHtml((d.saleAreas||[]).flatMap((r)=>CITY_MAP[r]||[]), d.directAreas||[], d.cityQ||d.directCityQ||'')}
    ...
    } else if (type === 'l1-disable-pending') {
      const ids = payload.ids || [];
      const l2Ids = payload.l2Ids || [];
      const list = ids.map((xid) => db.agentsL1.find((a) => a.id === xid)).filter(Boolean);
      const l2list = l2Ids.map((xid) => db.agentsL2.find((a) => a.id === xid)).filter(Boolean);
      title = '代理停用待处理';
      const card = (a, kind, action) => {
        const c = kind === 'l1' ? l1DisableCosign(a) : l2DisableCosign(a);
        const who = c.admin1 ? (c.admin1By || 'admin') : (c.admin2By || 'admin2');
        const at = c.admin1At || c.admin2At || '—';
        return `<div class="page-card" style="margin-top:10px">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
              <div>
                <strong>${escapeHtml(a.name)}</strong> ${tag(kind === 'l1' ? '一级' : '二级', 'gray')} ${tag('停用待会签', 'orange')}
                <p class="muted" style="margin:6px 0 0">${escapeHtml(who)} 已于 ${escapeHtml(at)} 申请停用</p>
              </div>
              <button class="btn btn-danger" data-action="${action}" data-id="${a.id}">确认停用会签</button>
            </div>
          </div>`;
      };
      body = `<div class="alert alert-warn">停用需两名管理员全部会签确认。仅一名管理员确认时，该代理仍可正常使用（未停用）。此待办优先级最高。</div>
        ${list.map((a) => card(a, 'l1', 'sign-l1-disable')).join('')}
        ${l2list.map((a) => card(a, 'l2', 'sign-l2-disable')).join('')}
        ${!list.length && !l2list.length ? emptyHint('暂无待会签') : ''}`;
      foot = `<button class="btn" data-action="close-modal">稍后处理</button>`;
    } else if (type === 'edit-l2') {
      const a = draft;
      title = '编辑二级代理';
      ...
      body = `<div class="form-grid">
          ...
          <div class="form-field span-2"><label>所属一级（更改绑定）</label>
            <select class="field-input" id="f-parent">${db.agentsL1.filter((x)=>x.status==='启用'||x.id===a.parentId).map((x)=>`<option value="${x.id}" ${x.id===a.parentId?'selected':''}>${escapeHtml(x.name)}</option>`).join('')}</select>
          </div>
      ...
      ${citySearchPickerHtml(cityOpts, d.areas || [], d.cityQ || '', 'data-toggle-city', 'select-all-city', {
        label: '授权城市（点击多选）',
        allLabel: '全选',
        empty: '该一级暂无可售城市，请先在一级详情维护可销售范围',
        note: `${dropped.length ? `<p class="muted" style="margin-top:6px">原城市「${escapeHtml(dropped.join('、'))}」不在当前一级可售范围内，已取消勾选</p>` : ''}<p class="muted" style="margin-top:6px">切换绑定一级后，可选城市会随一级可销售范围更新</p>`,
      })}`;
      ...
      const lifePanel = `<aside class="side-note-panel">
          <h4>完整流转（${life.length}）</h4>
          <div class="mini-timeline" style="max-height:420px;overflow:auto">${life.map((e)=>`<div class="mini-tl-item type-${e.type||''}"><div class="mini-tl-dot"></div><div><div class="mini-tl-title">${escapeHtml(e.title)}</div><div class="mini-tl-desc">${escapeHtml(e.desc||'')}</div><div class="mini-tl-time">${escapeHtml(e.time)}</div></div></div>`).join('')}</div>
        </aside>`;
      body = ui.mode === 'mini'
        ? `${snFields}${lifePanel}`
        : `<div class="detail-split"><div>${snFields}</div>${lifePanel}</div>`;
      foot = editing
      ...
      if (type === 'audit-po') {
        ...
        body = `<div class="detail-grid">
          ...
          <div><span>状态</span>${poStatusTag(p.status)}</div>
        ...
        foot = `<button class="btn" data-action="close-modal">关闭</button>
          ${ui.mode !== 'mini' && ['pending', 'cosigning'].includes(p.status)
            ? `<button class="btn btn-primary" data-action="open-audit-po" data-id="${p.id}">去审核</button>`
            : ''}`;
      }
      ...
      const tab = exceptionTab(e);
      const info = exceptionAgentInfo(e);
      const cust = exceptionCustomer(e);
      const explainLabel = exceptionExplainLabel(tab);
      const explainFull = exceptionExplainText(e, tab);
      ...
      const isSn = e.target && String(e.target).startsWith('RL');
      body = `<div class="detail-grid">
        <div><span>时间</span>${escapeHtml(e.time)}</div>
        <div><span>维度</span>${tag(exceptionTabLabel(tab))}</div>
        <div><span>一级代理</span>${escapeHtml(info.l1Name)}</div>
        <div><span>二级代理</span>${escapeHtml(tab === 'activate-direct' ? '—' : info.l2Name)}</div>
        <div><span>对象</span>${escapeHtml(e.target)}</div>
        <div><span>状态</span>${tag(displayExStatus(e), exStatusTone(displayExStatus(e)))}</div>
        <div class="span-2"><span>详情</span>${escapeHtml(e.detail)}</div>
        <div class="span-2"><span>${escapeHtml(explainLabel)}</span>${escapeHtml(explainFull || '—')}</div>
      </div>
      ${cust ? `<h4 style="margin-top:12px">销售客户信息</h4>
        <div class="detail-grid">
          <div><span>姓名</span>${escapeHtml(cust.name || '—')}</div>
          <div><span>性别</span>${escapeHtml(cust.gender || '—')}</div>
          <div><span>电话</span>${escapeHtml(cust.phone || '—')}</div>
        </div>` : ''}
      ${/客户信息重复/.test(e.type) ? `<h4 style="margin-top:12px">重复客户关联 SN</h4>
        <div class="page-card table-wrap"><table class="data">
          <thead><tr><th>SN</th><th>姓名</th><th>性别</th><th>电话</th><th>地址</th></tr></thead>
          <tbody>${related.map((s)=>`<tr class="row-clickable" data-row-action="view-sn" data-id="${s.sn}">
            <td>${escapeHtml(s.sn)}</td>
            <td>${escapeHtml(s.user?.name||'—')}</td>
            <td>${escapeHtml(s.user?.gender||'—')}</td>
            <td>${escapeHtml(s.user?.phone||'—')}</td>
            <td>${escapeHtml(s.user?.addr||'—')}</td>
          </tr>`).join('') || `<tr><td colspan="5">${emptyHint()}</td></tr>`}</tbody>
        </table></div>` : ''}`;
      foot = `<button class="btn" data-action="close-modal">关闭</button>
        ${isSn ? `<button class="btn" data-action="open-view-sn" data-id="${escapeHtml(e.target)}">看SN</button>` : ''}
        ${/客户信息重复/.test(e.type) ? `<button class="btn" data-action="view-dup-customer" data-id="${e.id}">查看重复</button>` : ''}
        ${isExOpen(e) ? `<button class="btn btn-primary" data-action="close-exception" data-id="${e.id}">处理</button>
        <button class="btn btn-danger" data-action="delete-exception" data-id="${e.id}">删除</button>` : ''}`;
      ...
      title = '离开异常详情';
      body = `<p>当前筛选仍有 <strong>${payload.count || 0}</strong> 条<strong>待处理</strong>异常。</p>
      ...
      title = '异常标准配置';
      body = `<p class="muted" style="margin-bottom:12px">全局倍数针对所有一级和二级分销。一级分销给二级不触发库存异常；二级详情可单独设严格/软报警。</p>
        <div class="form-grid">
      ...
      body = `<p>${escapeHtml(e?.type || '')} · ${escapeHtml(e?.target || '')}</p>
        ...
        <div class="form-field"><label>${escapeHtml(exceptionExplainLabel(exceptionTab(e)))}</label><textarea class="field-input" id="f-explain" rows="3">${escapeHtml(exceptionExplainText(e, exceptionTab(e)))}</textarea></div>`;
      ...
      const stdKits = (isSingle || isPart) ? [] : kitStdCombos(poProd);
      const bandOpts = productSizes(poProd);
      const beltOpts = productBelts(poProd);
      const customs = (isSingle || isPart) ? [] : (ui.modal.draft.customLines || []).map((c) => ({
        ...c,
      ...
      body = `<div class="alert alert-info">${escapeHtml(productComboNote(poProd))}</div>
        <div class="form-grid">
        <div class="form-field span-2"><label>商品</label><select class="field-input" id="f-pid">${kits.map((p)=>`<option value="${p.id}" ${p.id===poPid?'selected':''}>${escapeHtml(p.name)}${p.type==='single'?'（单品）':p.type==='part'?'（配件）':''}</option>`).join('')}</select></div>
        ${isPart || isSingle
          ? bandOpts.map((s)=>`<div class="qty-row span-2"><label>${isPart ? '规格' : '尺码'} ${escapeHtml(s)}</label><input type="number" class="field-input" data-size-qty="${s}" data-std-belt="" value="0" /></div>`).join('') || `<p class="muted span-2">请先维护${isPart ? '配件规格' : '单品尺码'}</p>`
          : (stdKits.map((k)=>`<div class="qty-row span-2"><label>${escapeHtml(k.label)}</label><input type="number" class="field-input" data-size-qty="${k.size}" data-std-belt="${k.belt}" value="0" /></div>`).join('') || `<p class="muted span-2">该商品未配置可用标品组合</p>`)}
        </div>
        ${isSingle || isPart ? '' : `
      ...
      const isCreateKit = type === 'create-product' && isKit;
      ...
      const kitCompsHtml = comps.map((c, ci) => {
        ...
        const createLabel = ci === 0 ? '组件1弹力带' : (ci === 1 ? '组件2腰带' : `组件${ci + 1}`);
        const sizeLabel = isCreateKit ? createLabel : (c.name || ('组件' + (ci + 1)));
        const nameBlock = isCreateKit
          ? `<div style="font-weight:600;margin-bottom:8px">${escapeHtml(createLabel)}</div>`
        ...
        return `<div class="form-field span-2" style="border:1px solid var(--border,#e8e8e8);border-radius:8px;padding:12px;margin-top:4px">
          ${nameBlock}
          <div style="${isCreateKit ? '' : 'margin-top:10px'}"><label>${escapeHtml(sizeLabel)}尺码 ${selectAllBtn('select-all-comp-size', '全选', allOn).replace('data-action="select-all-comp-size"', `data-action="select-all-comp-size" data-idx="${ci}"`)}</label>
            ${pool.length ? chips(pool, selected, 'data-toggle-comp-size').replace(/data-toggle-comp-size="/g, `data-toggle-comp-size="${ci}|`) : `<p class="muted" style="margin:8px 0 0">暂无尺码，请在下方添加</p>`}
      ...
      const stdHead = `<th>档位</th>${comps.map((c) => `<th>${escapeHtml(isCreateKit ? (c.name || (c.id === 'c0' ? '弹力带' : '腰带')) : c.name)}</th>`).join('')}<th>组合说明</th><th></th>`;
      ...
      const stdBlock = `<div class="form-field span-2"><label>标准套件配置（随上方已选尺码自动生成）
      ...
      body = `<div class="form-grid">
        <div class="form-field"><label>商品编号</label><input class="field-input" id="f-pcode" value="${escapeHtml(d.code || '')}" /></div>
        <div class="form-field"><label>商品名称</label><input class="field-input" id="f-pname" value="${escapeHtml(d.name || '')}" placeholder="${isSingle ? '如：护膝单品' : (isCreateKit ? '如：弹力带+腰带套件' : '如：袜子+鞋套套件')}" /></div>
        ${isCreateKit ? '' : `<div class="form-field span-2"><label>说明</label><input class="field-input" id="f-pnote" value="${escapeHtml(d.note || '')}" /></div>`}
        ${isCreateKit ? `${kitCompsHtml}${stdBlock}` : isKit ? `
          ...
          <div class="form-field span-2"><div class="alert alert-info" style="margin:0">可配置多个组件：先命名并<strong>勾选尺码</strong>，下方「标准套件组合」会按已选尺码<strong>自动生成全部组合</strong>。</div></div>
          ${kitCompsHtml}
      ...
      const stdKits = (isSingle || isPart) ? [] : kitStdCombos(cartProd);
      const singleSizes = (isSingle || isPart) ? productSizes(cartProd) : [];
      const nonstdOpts = (isSingle || isPart) ? [] : nonstdGradesForProduct(cartProd);
      ...
      const stdTotal = Object.values(d.stdQty || {}).reduce((n, q) => n + (Number(q) || 0), 0);
      const customTotal = (d.customRows || []).reduce((n, r) => n + (Number(r.qty) || 0), 0);
      title = channel === 'sales' ? '提交销售单（购物车）' : '提交采购单（购物车）';
      body = `<div class="alert alert-info">${escapeHtml(productComboNote(cartProd))}</div>
        <div class="form-grid">
          <div class="form-field span-2"><label>商品名称</label><select class="field-input" id="f-cart-pid">${kits.map((p)=>`<option value="${p.id}" ${p.id===cartPid?'selected':''}>${escapeHtml(p.name)}${p.type==='single'?'（单品）':p.type==='part'?'（配件）':''}</option>`).join('')}</select></div>
        ...
        ${isPart ? `
        <h4>配件规格（${singleSizes.length}）</h4>
        <div class="qty-list">
          ${singleSizes.map((s)=>`<div class="qty-row"><label>${escapeHtml(s)}</label>
            <input type="number" class="field-input" data-std-key="${s}" data-std-size="${s}" data-std-belt="" value="${d.stdQty[s]||0}" min="0" /></div>`).join('') || `<p class="muted">该配件未维护规格</p>`}
        </div>
        <div class="qty-total">总计 <strong class="num" id="std-qty-total">${stdTotal}</strong> 件配件</div>` : isSingle ? `
        ...
        <div class="qty-total">总计 <strong class="num" id="std-qty-total">${stdTotal}</strong> 件</div>` : `
        <h4>标准套件（${stdKits.length}）</h4>
        <div class="qty-list">
          ${stdKits.map((k)=>`<div class="qty-row"><label>${escapeHtml(k.label)}</label>
        ...
        <div class="qty-total">标准套件总计 <strong class="num" id="std-qty-total">${stdTotal}</strong> 件</div>
        <h4 style="margin-top:14px">非标套件</h4>
        ${nonstdOpts.length ? `<div class="nonstd-row">
        ...
        <p class="muted" style="margin:6px 0">非标仅展示「本商品已维护尺码」中排除标品后的组合。可多次添加。</p>` : `<p class="muted">该商品无可下非标组合（${escapeHtml(aName)}/${escapeHtml(bName)}尺码不足）</p>`}
      ...
      const mode = payload.mode || (ui.role === 'l2' ? 'cend' : 'l2_to_l1');
      ...
      const typeVal = cend ? 'user' : factory ? 'l1_to_factory' : 'l2_to_l1';
      ...
      body = `<div class="form-field"><label>退货理由</label><select class="field-input" id="f-rtype">${RETURN_REASONS.map((r)=>`<option value="${r.type}" ${reasonType===r.type?'selected':''}>${r.label}</option>`).join('')}</select></div>
        ...
        <div class="form-field"><label>类型</label>
          <input class="field-input" value="${typeLabel}" readonly /><input type="hidden" id="f-ttype" value="${typeVal}" />
        </div>
        <p class="mini-page-desc">${cend
          ? '校验销售方必须是当前代理；确认后转入本级库存，并打「已退货」标签。'
          : factory
            ? '确认后提交平台审核；通过后状态改为原厂在库，标签「已冻结」。'
            : '由一级填写，无需二级发起。确认后直接转入一级库存，并打「已退货」标签。'}</p>`;
      foot = `<button class="btn" data-action="close-modal">取消</button><button class="btn btn-primary" data-action="create-return-ok">确认</button>`;
    ...
    const wide = type === 'audit-po' || type === 'view-sn' || type === 'edit-sn' || type === 'edit-l1' || type === 'create-l1' || type === 'edit-l2' || type === 'rebind-l2' || type === 'order-cart' || type === 'view-exception' || type === 'create-product' || type === 'edit-product' || type === 'view-stock' || type === 'view-return' || type === 'view-customer' || type === 'create-customer' || type === 'edit-customer' || type === 'create-role';
  ...
  function validateReturnSns(sns, type) {
    const issues = [];
    let returnedTagN = 0;
    (sns || []).forEach((sn) => {
      const row = db.sns.find((x) => x.sn === sn);
      if (!row) { issues.push(`${sn} 不存在`); return; }
      if (snBusinessTags(row).includes('已退货')) returnedTagN += 1;
      if (type === 'l2_to_l1') {
        if (row.status !== 'l2') issues.push(`${sn} 不在二级库存（${snStatusMeta(row).label}）`);
        else if (row.l1Id !== currentL1Id()) issues.push(`${sn} 不属于本一级下属`);
      } else if (type === 'user') {
        if (row.status !== 'bound') issues.push(`${sn} 未销售，不能做终端退货`);
        else if (ui.role === 'l2') {
          if (row.l2Id !== currentL2Id()) issues.push(`${sn} 销售方不是当前二级代理`);
        } else if (ui.role === 'l1') {
          if (row.l2Id) issues.push(`${sn} 销售方是二级「${l2Name(row.l2Id)}」，请由该二级处理`);
          else if (row.l1Id !== currentL1Id()) issues.push(`${sn} 销售方不是当前一级`);
        }
      } else if (type === 'l1_to_factory') {
        if (snCanonicalStatus(row) !== 'l1' || row.l1Id !== currentL1Id()) {
          issues.push(`${sn} 不在本一级库存`);
        }
      }
    });
    return { issues, returnedTagN };
  }

  function applyReturnInbound(r) {
    (r.sns || []).forEach((sn) => {
      const row = db.sns.find((x) => x.sn === sn);
      if (!row) return;
      if (r.type === 'user') {
        const toL2 = r.fromRole === 'l2' || String(r.fromId || '').startsWith('L2');
        ...
        if (toL2) {
          row.status = 'l2';
          row.l2Id = r.fromId;
          db.stockLogs.unshift({ id: uid('H'), agentType: 'l2', agentId: r.fromId, productId: row.productId, size: row.size, delta: 1, reason: '终端退货入库', time: nowStr(), ref: r.no });
        } else {
          row.status = 'l1';
          row.l2Id = null;
          db.stockLogs.unshift({ id: uid('H'), agentType: 'l1', agentId: r.fromId || row.l1Id, productId: row.productId, size: row.size, delta: 1, reason: '终端退货入库', time: nowStr(), ref: r.no });
        }
        pushSnEvent(row, '终端用户退货', r.no, 'return');
        ...
        row.returnAt = nowStr();
        row.factoryAt = nowStr();
        pushSnEvent(row, '一级退回原厂', r.no, 'factory');
  ...
  function approveReturn(id) {
    ...
    applyReturnInbound(r);
    addLog(`退货审批通过 ${r.no}`);
    saveStore();
    toast(r.type === 'l1_to_factory' ? '已通过；SN 已回原厂在库并打已冻结标签' : '已通过并转入库存');
    render();
  }
  ...
  function analyzeDirectBind(sn, phone, addr, ipRegion) {
    ...
    if (row.frozen || (row.tags || []).includes('已冻结')) return { err: '已冻结 SN 不可扫码' };
    if (!snInOwnWarehouse(row) && !(row.status === 'bound' && row.reIn && row.resale)) {
      return { err: `不在本${ui.role === 'l2' ? '二级' : '一级'}仓库：${snStatusMeta(row).label}` };
    }
  ...
  function doDirectBind(sn, phone, addr, ipRegion, extra = {}) {
    ...
    const hadReturned = snBusinessTags(row).includes('已退货');
    row.status = 'bound';
    ...
    db.sales.unshift({
      ...
      customer: { phone, addr, phoneLoc, name: extra.name || '', gender: extra.gender || '', age: extra.age || '', note: extra.note || '' },
    });
      ...
      <div class="panel">
        <aside class="sidebar"><div class="sidebar-scroll">
          ${menus.map((g)=>`<div class="nav-group-title">${g.group}</div>${g.items.map((it)=>
            `<button class="nav-item ${sidebarActiveId()===it.id?'active':''}" data-go="${it.id}">
  ...
  function applySalesAlias(id) {
    if (id === 'l1-sales-detail') {
      ...
      if (ui.tabs['l1-sales'] === 'direct') {
        ui.filters.customers = {
          ...(ui.filters.customers || {}),
          from: f.from || monthStart(),
          to: f.to || todayDate(),
          l1: f.l1Id || ui.filters.sales?.l1 || '',
      ...
      if (ui.tabs['l1-sales'] === 'distribute') ui.tabs.sales = 'distribute';
      return 'sales';
    }
  ...
  function handleAction(action, el) {
    ...
    switch (action) {
      ...
      case 'logout':
        ...
        break;
      case 'sign-l1-disable': {
        const a = db.agentsL1.find((x) => x.id === id);
        if (!a) break;
        const r = signL1Disable(a);
        saveStore();
        const restL1 = pendingL1DisableForAccount(ui.account);
        const restL2 = pendingL2DisableForAccount(ui.account);
        if (r === 'done' && !restL1.length && !restL2.length) closeModal();
        else {
          if (ui.modal?.type === 'l1-disable-pending') {
            ui.modal.payload.ids = restL1.map((x) => x.id);
            ui.modal.payload.l2Ids = restL2.map((x) => x.id);
          }
          render();
        }
        break;
      }
      case 'sign-l2-disable': {
        const a = db.agentsL2.find((x) => x.id === id);
        if (!a) break;
        const r = signL2Disable(a);
        saveStore();
        const restL1 = pendingL1DisableForAccount(ui.account);
        const restL2 = pendingL2DisableForAccount(ui.account);
        if (r === 'done' && !restL1.length && !restL2.length) closeModal();
      ...
      case 'create-l1-ok': {
        ...
        d.disableCosign = { admin1: false, admin2: false };
        const { username: _u, password: _p, directCityQ: _q, ...row } = d;
      ...
      case 'disable-l1': {
        ...
        if (!a) break;
        if (a.status === '停用') {
          confirmDialog('确认启用该一级代理？', 'disable-l1-ok', { id }, { title: '一级代理状态确认' });
        } else {
          const pending = l1DisablePending(a);
          const msg = pending
            ? `另一管理员已申请停用「${a.name}」。你确认后将正式停用，下属法人二级进入待分配。仅一名管理员确认时该一级仍可正常使用。`
            : `停用「${a.name}」需两名管理员全部会签确认。仅一名管理员确认时，该一级仍可正常使用（未停用）。`;
          confirmDialog(msg, 'disable-l1-ok', { id }, { title: '一级代理停用会签', danger: true, okText: pending ? '确认停用会签' : '申请停用' });
        }
        break;
      }
      case 'disable-l2': {
        ...
        if (!a) break;
        if (a.status === '停用') {
          confirmDialog('确认启用该二级代理？', 'disable-l2-ok', { id }, { title: '二级代理状态确认' });
        } else {
          const pending = l2DisablePending(a);
          const msg = pending
            ? `另一管理员已申请停用「${a.name}」。你确认后将正式停用。仅一名管理员确认时该二级仍可正常使用。`
            : `停用「${a.name}」需两名管理员全部会签确认。仅一名管理员确认时，该二级仍可正常使用（未停用）。`;
          confirmDialog(msg, 'disable-l2-ok', { id }, { title: '二级代理停用会签', danger: true, okText: pending ? '确认停用会签' : '申请停用' });
        }
        break;
      }
      case 'confirm-cancel':
      ...
      case 'confirm-ok': {
        ...
        if (act === 'disable-l1-ok') {
          ...
          render();
        } else if (act === 'po-delete-ok') {
          const idx = db.purchases.findIndex((x)=>x.id===pid);
          if (idx >= 0) {
          ...
          if (e) {
            if (signExceptionProcess(e)) saveStore();
          }
          render();
        } else if (act === 'delete-exception-ok') {
          const idx = db.exceptions.findIndex((x)=>x.id===pid);
          if (idx >= 0) {
            const e = db.exceptions[idx];
            db.exceptions.splice(idx, 1);
            addLog(`删除异常 ${e.type} ${e.target}`, 'exception');
            saveStore();
            toast('已删除');
          }
          ui.modal = null;
          ...
          ui.loggedIn = false; ui._l1DisablePrompted = false; persistSession(); ui.modal = null; ui.confirm = null; render();
          ...
          doDirectBind(p.sn, p.phone, p.addr, p.region, {
            name: p.name || '', gender: p.gender || '', age: p.age || '', note: p.note || '',
          });
          render();
      ...
      case 'save-l2': {
        ...
        const oldParent = i >= 0 ? db.agentsL2[i].parentId : d.parentId;
        if (i>=0) db.agentsL2[i] = d;
        addLog(d.parentId !== oldParent ? `更改绑定二级 ${d.name} → ${l1Name(d.parentId)}` : `编辑二级 ${d.name}`);
      ...
      case 'po-delete':
        ...
        confirmDialog(`确认删除采购单 ${ui.modal.draft?.no || ''}？删除后不可恢复。`, 'po-delete-ok', { id: ui.modal.draft.id }, { title: '删除采购单', danger: true, okText: '确认删除' });
        break;
      case 'po-confirm':
        syncDraftFromAuditDom();
      ...
      case 'save-ex-rules': {
        ...
        addLog(`更新异常标准 全局倍数${db.exceptionMultiplier}`);
      ...
      case 'leave-ex-done': {
        ...
        ids.forEach((eid) => {
          ...
          if (e && isExOpen(e)) e.status = exClosedStatus(e);
        });
      ...
      case 'save-ex-explain': {
        ...
        if (e) {
          const text = $('#f-explain')?.value?.trim() || '';
          if (exceptionTab(e) === 'activate-dist') e.explainL2 = text;
          else e.explain = text;
          addLog(`填写${exceptionExplainLabel(exceptionTab(e))} ${e.target}`);
          saveStore();
        }
        closeModal(); toast('解释已保存');
        openModal('view-exception', { id });
        break;
      }
      ...
      case 'reassign-frozen-ok': {
        ...
        confirmDialog(`确认将已冻结 SN「${id}」分配给「${l1Name(l1Id)}」并解除冻结？`, 'reassign-frozen-confirm-ok', { id, l1Id }, { title: '原厂在库重分配', okText: '确认分配' });
        break;
      }
      case 'approve-return': {
        const r = db.returns.find((x)=>x.id===id);
      ...
      case 'close-exception': {
        const e = db.exceptions.find((x)=>x.id===id);
        const stock = e && exceptionDim(e) === 'stock';
        confirmDialog(
          stock ? '确认处理该异常？处理后标记为已处理，无需会签。' : '确认处理该异常？需两名管理员会签后标记为已完成。',
          'close-ex-ok', { id }, { title: '异常处理确认', okText: '确认处理' },
        );
        break;
      }
      case 'delete-exception':
        confirmDialog('确认删除该异常？删除后不可恢复。', 'delete-exception-ok', { id }, { title: '删除异常', danger: true, okText: '确认删除' });
        break;
      case 'save-ex-mult': {
      ...
      case 'mini-direct-bind': {
        ...
        const extra = {
          name: $('#direct-name')?.value?.trim() || '',
          gender: $('#direct-gender')?.value || '',
          age: $('#direct-age')?.value?.trim() || '',
          note: $('#direct-note')?.value?.trim() || '',
        };
        ui.form.directName = extra.name;
        ui.form.directGender = extra.gender;
        ui.form.directAge = extra.age;
        ui.form.directNote = extra.note;
        const payload = { sn, phone, addr, region, ...extra };
      ...
      case 'mini-direct-step1': {
        ...
        if (!row) {
          reportScanNotInStock(sn, null);
          return toast('SN 不存在', 'err');
        }
        if (row.frozen || (row.tags || []).includes('已冻结')) return toast('已冻结 SN 不可扫码', 'err');
        if (!snInOwnWarehouse(row)) {
          reportScanNotInStock(sn, row);
          return toast(`不在本${ui.role === 'l2' ? '二级' : '一级'}仓库：${snStatusMeta(row).label}`, 'err');
        }
        ui.form.directSn = sn;
        ui.directStep = 2;
      ...
      case 'select-all-direct': {
        ...
        const q = ui.modal.draft.cityQ || ui.modal.draft.directCityQ || '';
      ...
      case 'select-all-city': {
        ...
        const q = ui.modal.draft.cityQ || $('#f-city-q')?.value || '';
        const target = filterCitiesByQ(opts, q);
        if (!target.length) return toast(q ? '无匹配城市' : '当前一级无可选城市', 'warn');
      ...
      case 'open-create-product': {
        ...
        if (ptype === 'part') {
          ...
          const seed = {
            type: 'kit',
            components: [
              { id: 'c0', name: '弹力带', pool: [], sizes: [] },
              { id: 'c1', name: '腰带', pool: [], sizes: [] },
            ],
            stdCombos: [],
            bundleSingles: [],
            status: '上架',
      ...
      case 'save-product': {
        ...
        if (d.type === 'kit') {
          const comps = productComponents(d);
          if (ui.modal.type === 'create-product') {
            if (comps[0]) comps[0].name = (comps[0].name || '').trim() || '弹力带';
            if (comps[1]) comps[1].name = (comps[1].name || '').trim() || '腰带';
          }
      ...
      case 'cart-submit': {
        ...
        const cartPid = $('#f-cart-pid')?.value || orderableProducts()[0]?.id;
      ...
      case 'create-po-ok': {
        const productId = $('#f-pid')?.value;
        const poProd = orderableProducts().find((p) => p.id === productId) || orderableProducts()[0];
        const isSingle = poProd?.type === 'single';
        const isPart = poProd?.type === 'part';
        const lines = [];
        document.querySelectorAll('[data-size-qty]').forEach((inp) => {
      ...
      case 'mini-create-return': openModal('create-return', { mode: 'l2_to_l1' }); break;
      case 'mini-create-factory-return': openModal('create-return', { mode: 'factory' }); break;
      ...
      case 'create-return-ok': {
        ...
        const mode = ui.modal?.payload?.mode;
        const type = mode === 'cend' ? 'user' : mode === 'factory' ? 'l1_to_factory' : ($('#f-ttype')?.value || 'l2_to_l1');
        if (!sns.length) return toast('请填写 SN', 'err');
        const v = validateReturnSns(sns, type);
        if (v.issues.length) return toast(v.issues[0], 'err');
        const labels = { l2_to_l1: '二级退一级', l1_to_factory: '一级退原厂', user: '终端用户退货' };
        const immediate = type === 'l2_to_l1' || type === 'user';
        confirmDialog(
          `已比对：${sns.length} 个 SN 中 ${v.returnedTagN} 个带「已退货」标签。\n确认后${immediate ? '立即转入库存' : '提交平台审核'}？`,
          'create-return-confirm-ok',
          {
  ...
  function finishScanConfirmSo(id) {
    ...
    s.scanned.forEach((sn) => {
      ...
      if (snBusinessTags(row).includes('已退货')) addSnTag(row, '再入库');
  ...
  function finishReassignFrozen(sn, l1Id) {
    ...
    row.tags = (row.tags || []).filter((t) => t !== '冷冻' && t !== '已冻结');
    pushSnEvent(row, '原厂在库重分配', l1Name(l1Id), 'reassign');
    addLog(`已冻结SN重分配 ${sn} → ${l1Name(l1Id)}`);
    saveStore(); ui.modal = null; toast('已分配回一级库存'); render();
  }
  ...
  function finishCreateReturn(payload) {
    ...
    if (payload.immediate) {
      applyReturnInbound(rec);
      addLog(`售后确认 ${rec.no} · ${rec.typeLabel}`);
      toast(`已确认：${rec.returnedTagN} 个带已退货标签，已转入库存`);
    } else {
      addLog(`提交退货 ${rec.no}`);
      toast('退货已提交，等待平台审核');
    }
    saveStore(); ui.modal = null; render();
  }
  ...
  function bindEvents() {
    ...
    document.querySelectorAll('[data-go]').forEach((el) => el.addEventListener('click', (e) => {
      ...
      if (dest === 'customers') {
        const set = el.getAttribute('data-set-filter') || '';
        if (!set.includes('customers:directOnly')) {
          ui.filters.customers = ui.filters.customers || {};
          ui.filters.customers.directOnly = '';
        }
      }
      ...
      if (back) {
        ui.backRoute = back;
        ui.backModal = null;
      } else if (dest !== 'agent-l1-detail' && dest !== 'agent-l2-detail' && dest !== ui.route) {
        ui.backRoute = '';
        ui.backModal = null;
      }
      navigate(dest);
    ...
    document.querySelectorAll('[data-tab]').forEach((el) => el.addEventListener('click', () => {
      ...
      if (key === 'sales' && id === 'direct') {
        const sf = ui.filters.sales || {};
        ui.filters.customers = {
          ...(ui.filters.customers || {}),
          from: sf.from || monthStart(),
          to: sf.to || todayDate(),
          l1: sf.l1 || '',
          l2: '',
          directOnly: '1',
      ...
      ui.tabs[key] = id;
      if (key === 'exception' && ui.filters.exception) {
        ui.filters.exception.type = '';
        if (id === 'activate-direct') ui.filters.exception.l2 = '';
      }
      render();
    }));
    ...
    if (ui.modal?.type === 'rebind-l2' || ui.modal?.type === 'edit-l2') {
    ...
    $('#modal-mask')?.addEventListener('click', (e) => {
      if (e.target.id === 'modal-mask' && !ui.confirm && ui.modal?.type !== 'l1-disable-pending') closeModal();
    });
    ...
    if (ui.modal?.type === 'order-cart') {
      ...
      document.querySelectorAll('[data-std-key], [data-bundle-qty]').forEach((inp) => {
        const sync = () => {
          ...
          const el = document.getElementById('std-qty-total');
    ...
    if (ui.modal?.type === 'audit-po') {
      const refreshAuditMatch = () => {
        ...
        document.querySelectorAll('.audit-line').forEach((el) => {
          let lineSeg = 0;
          el.querySelectorAll('[data-seg-count]').forEach((td) => {
            const si = td.getAttribute('data-seg-count');
            const from = el.querySelector(`[data-seg-from="${si}"]`)?.value || '';
            const to = el.querySelector(`[data-seg-to="${si}"]`)?.value || '';
            const n = segmentCount(from && to ? `${from}-${to}` : from);
            td.textContent = n;
            lineSeg += n;
          });
          const kind = el.getAttribute('data-line-kind');
          const qtyEl = document.querySelector(`[data-audit-qty-sum="${kind}"]`);
          const segEl = document.querySelector(`[data-audit-seg-sum="${kind}"]`);
          if (qtyEl) {
            let q = 0;
            document.querySelectorAll(`.audit-line[data-line-kind="${kind}"] [data-po-field="qty"]`).forEach((inp) => { q += Number(inp.value) || 0; });
            qtyEl.textContent = q;
          }
          if (segEl) {
            let s = 0;
            document.querySelectorAll(`.audit-line[data-line-kind="${kind}"] [data-seg-count]`).forEach((td) => { s += Number(td.textContent) || 0; });
            segEl.textContent = s;
          }
          void lineSeg;
        });
